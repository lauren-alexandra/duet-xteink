...
void testTopLevelReleaseListUsesFirstRelease() {
  printf("testTopLevelReleaseListUsesFirstRelease...\n");
  ...
  const char* json = R"([
    {
      ...
      "assets": [{
        "name": "firmware.bin",
        "browser_download_url": "https://example.com/alpha2.bin",
        "size": 6001
      }]
    },
    {
      ...
      "assets": [{
        "name": "firmware.bin",
        "browser_download_url": "https://example.com/alpha1.bin",
        "size": 6000
      }]
    }
  ])";
  ...
  ASSERT_TRUE(p.foundTag());
  ASSERT_TRUE(p.foundFirmware());
  ASSERT_STREQ(p.getTagName(), "v0.1.0-alpha.2");
  ASSERT_STREQ(p.getFirmwareUrl(), "https://example.com/alpha2.bin");
  ASSERT_EQ(p.getFirmwareSize(), 6001u);
...
int main() {
  ...
  testTopLevelReleaseListUsesFirstRelease();
