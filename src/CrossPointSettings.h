...
#include <Epub/BionicReadingMode.h>
...
class CrossPointSettings {
 ...
 public:
  ...
  enum FILE_BROWSER_DISPLAY {
    ...
    FILE_BROWSER_DISPLAY_COVERS = 2,
    FILE_BROWSER_DISPLAY_CAROUSEL = 3,
    FILE_BROWSER_DISPLAY_COUNT
  };

  // CrumBLE-style square bookshelf density used by the file browser's cover grid.
  enum FILE_BROWSER_GRID_LAYOUT {
    FILE_BROWSER_GRID_2X2 = 0,
    FILE_BROWSER_GRID_3X3 = 1,
    FILE_BROWSER_GRID_4X4 = 2,
    FILE_BROWSER_GRID_LAYOUT_COUNT
  };

  enum FILE_BROWSER_SORT {
    FILE_BROWSER_SORT_AUTHOR = 0,
    FILE_BROWSER_SORT_TITLE = 1,
  ...
  enum READER_REFRESH_MODE : uint8_t {
  ...
  // Dashboard Home stat picker. Values are persisted in settings JSON, so
  // they are stable IDs: append new stats at the end, never renumber.
  enum HOME_STAT {
    HOME_STAT_NONE = 0,
    HOME_STAT_BOOK_TIME = 1,
    HOME_STAT_TIME_LEFT = 2,
    HOME_STAT_PROGRESS = 3,
    HOME_STAT_DAILY_AVG = 4,
    HOME_STAT_PAGES_PER_MIN = 5,
    HOME_STAT_SESSIONS = 6,
    HOME_STAT_AVG_SESSION = 7,
    HOME_STAT_DAYS_READING = 8,
    HOME_STAT_EST_FINISH = 9,
    HOME_STAT_STREAK = 10,
    HOME_STAT_READER_TYPE = 11,
    HOME_STAT_TOTAL_TIME_DEVICE = 12,
    HOME_STAT_TOTAL_TIME_ALL = 13,
    HOME_STAT_BOOKS_DEVICE = 14,
    HOME_STAT_BOOKS_ALL = 15,
    HOME_STAT_TODAY_TIME = 16,
    HOME_STAT_TOTAL_SESSIONS = 17,
    HOME_STAT_COUNT
  };
  ...
  enum SLEEP_SCREEN_CYCLE_CLICKS : uint8_t {
    SLEEP_CYCLE_OFF = 0,
    SLEEP_CYCLE_ONE_CLICK = 1,
    SLEEP_CYCLE_TWO_CLICKS = 2,
    SLEEP_CYCLE_THREE_CLICKS = 3,
    SLEEP_SCREEN_CYCLE_CLICKS_COUNT
  };

  // Sleep screen settings
  uint8_t sleepScreen = DARK;
  // Number of brief Power presses required to show another custom sleep image.
  uint8_t sleepScreenCyclePowerClicks = SLEEP_CYCLE_THREE_CLICKS;
  ...
  // Awake power-button click actions. Existing single-press behavior is preserved on migration.
  uint8_t shortPwrBtn = IGNORE;
  uint8_t doublePwrBtn = FORCE_REFRESH;
  uint8_t triplePwrBtn = TOGGLE_BOOKMARK;
  ...
  // Optional page-turn waveform override. Auto preserves the cadence above.
  ...
  // Bionic Reading mode: Off, Normal, or a shorter Subtle bold prefix.
  uint8_t bionicReadingEnabled = BionicReadingMode::Off;
  ...
  // Stable selected SD-font point size. fontSize remains the current family's list index.
  ...
  // File browser display style. Cover modes apply only inside book-only shelves.
  uint8_t fileBrowserDisplay = FILE_BROWSER_DISPLAY_1_LINE;
  // Cover-grid density. 2x2 preserves the historical four-cover view.
  uint8_t fileBrowserGridLayout = FILE_BROWSER_GRID_2X2;
  // Book library sort. Author follows Lauren's author-first filenames; Title parses the title after " -- ".
  uint8_t fileBrowserSort = FILE_BROWSER_SORT_AUTHOR;
  ...
  uint8_t readingGoalMinutes = 20;
  // Open the freshly committed book statistics before returning Home after reading.
  uint8_t showStatsAfterReading = 0;
  // Show a one-time alert when newly earned achievements are detected on Home.
  uint8_t achievementPopups = 1;
  // Image rendering mode in EPUB reader
  uint8_t imageRendering = IMAGES_DISPLAY;
  // Long-press Confirm (menu button) quick action in reader.
  uint8_t longPressMenuAction = LONG_MENU_TOGGLE_BOOKMARK;
  ...
  // Dashboard Home stat slots (side column rows top-to-bottom, then the two
  // footer anchors). Defaults mirror the X3's original layout on both devices.
  uint8_t homeStatSlot1 = HOME_STAT_BOOK_TIME;
  uint8_t homeStatSlot2 = HOME_STAT_TIME_LEFT;
  uint8_t homeStatSlot3 = HOME_STAT_PROGRESS;
  uint8_t homeStatSlot4 = HOME_STAT_DAILY_AVG;
  uint8_t homeStatSlot5 = HOME_STAT_PAGES_PER_MIN;
  uint8_t homeStatSlot6 = HOME_STAT_DAYS_READING;
  uint8_t homeStatSlot7 = HOME_STAT_EST_FINISH;
  uint8_t homeFooterLeft = HOME_STAT_STREAK;
  uint8_t homeFooterRight = HOME_STAT_READER_TYPE;
  // Reading Home theme's four-tile stats strip.
  uint8_t homeStrip1 = HOME_STAT_TODAY_TIME;
  uint8_t homeStrip2 = HOME_STAT_STREAK;
  uint8_t homeStrip3 = HOME_STAT_TOTAL_TIME_DEVICE;
  uint8_t homeStrip4 = HOME_STAT_TOTAL_SESSIONS;
  ...
  static constexpr uint8_t MIN_READING_GOAL_MINUTES = 5;
  static constexpr uint8_t MAX_READING_GOAL_MINUTES = 120;
  static constexpr uint8_t READING_GOAL_STEP_MINUTES = 5;
