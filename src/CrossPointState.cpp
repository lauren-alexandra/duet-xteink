...
constexpr char STATE_FILE_BIN[] = DUET_STATE_ROOT_PATH "/state.bin";
constexpr char STATE_FILE_JSON[] = DUET_STATE_ROOT_PATH "/state.json";
constexpr char STATE_FILE_BAK[] = DUET_STATE_ROOT_PATH "/state.bin.bak";
}  // namespace
...
bool CrossPointState::saveToFile() const {
  ...
  const bool ok = JsonSettingsIO::saveState(*this, STATE_FILE_JSON);
  if (ok) persistedOpenEpubPath = openEpubPath;
  return ok;
}

bool CrossPointState::loadFromFile() {
  // Try JSON first
  if (Storage.existsForRead(STATE_FILE_JSON)) {
    String json = Storage.readFile(STATE_FILE_JSON);
    if (!json.isEmpty()) {
      std::lock_guard<std::mutex> lock(_mutex);
      const bool ok = JsonSettingsIO::loadState(*this, json.c_str());
      if (ok) persistedOpenEpubPath = openEpubPath;
      return ok;
    }
  }
  ...
  if (Storage.existsForRead(STATE_FILE_BIN)) {
