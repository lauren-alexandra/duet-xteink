...
class MappedInputManager {
 public:
  ...
  void update() const;
  // Long cooperative tasks use this to poll for cancellation while keeping
  // the detected edge available to the next normal app loop.
  void updatePreservingEvents() const;
  ...
  bool wasShortPowerActionResolved() const { return powerButtonClicks.finalizedClicks() > 0; }
  uint8_t getResolvedShortPowerAction() const;
  void cancelPowerButtonClicks() { powerButtonClicks.cancel(); }
 ...
 private:
  ...
  mutable PowerButtonClickDetector powerButtonClicks;
#ifdef SIMULATOR
