...
constexpr uint32_t DICTIONARY_SCAN_YIELD_INTERVAL = 64;

void yieldDuringDictionaryScan(const uint32_t itemCount) {
  if (itemCount == 0 || itemCount % DICTIONARY_SCAN_YIELD_INTERVAL != 0) return;
  vTaskDelay(1);
#ifndef SIMULATOR
  esp_task_wdt_reset();
#endif
...
void DictionaryStore::loadConfig() {
  ...
  if (!Storage.existsForRead(CONFIG_PATH)) return;
...
bool DictionaryStore::loadCheckpointCache(DictionaryEntry& entry) {
  ...
  auto discardInvalidCache = [&entry](HalFile* file, const char* reason) {
    if (file) file->close();
    entry.checkpoints.clear();
    entry.ordinals.clear();
    entry.totalWords = 0;
    Storage.remove(entry.cachePath.c_str());
    Storage.remove((entry.cachePath + ".tmp").c_str());
    LOG_INF("DICT", "Discarded invalid index cache for %s: %s", entry.name.c_str(), reason);
    return false;
  };
  ...
  if (file.read(header, sizeof(header)) != static_cast<int>(sizeof(header))) {
    return discardInvalidCache(&file, "short header");
  }

  if (readU32(header) != CACHE_MAGIC || readU32(header + 4) != entry.idxFileSize) {
    return discardInvalidCache(&file, "stale header");
  }

  const uint32_t count = readU32(header + 8);
  entry.totalWords = readU32(header + 12);
  const uint64_t expectedSize = 16ULL + static_cast<uint64_t>(count) * sizeof(uint32_t) * 2ULL;
  if (count == 0 || count > MAX_CHECKPOINT_COUNT || entry.totalWords == 0 || expectedSize != file.fileSize()) {
    return discardInvalidCache(&file, "invalid size or counts");
  }
  if (!hasHeapHeadroom(static_cast<size_t>(count) * sizeof(uint32_t) * 2, HEAP_SMALL_GUARD_BYTES)) {
    file.close();
    entry.totalWords = 0;
    return false;
  }
  ...
  for (uint32_t i = 0; i < count; ++i) {
    if (file.read(buf, 4) != 4) {
      return discardInvalidCache(&file, "truncated checkpoints");
    }
    entry.checkpoints.push_back(readU32(buf));
  }
  for (uint32_t i = 0; i < count; ++i) {
    if (file.read(buf, 4) != 4) {
      return discardInvalidCache(&file, "truncated ordinals");
    }
    entry.ordinals.push_back(readU32(buf));
  }
  file.close();

  if (entry.checkpoints.size() != entry.ordinals.size() || entry.checkpoints.front() != 0 ||
      entry.ordinals.front() != 0 || entry.ordinals.back() >= entry.totalWords) {
    return discardInvalidCache(nullptr, "invalid checkpoint bounds");
  }
  for (size_t i = 0; i < entry.checkpoints.size(); ++i) {
    if (entry.checkpoints[i] >= entry.idxFileSize || entry.ordinals[i] % CHECKPOINT_INTERVAL != 0 ||
        (i > 0 && (entry.checkpoints[i] <= entry.checkpoints[i - 1] ||
                   entry.ordinals[i] <= entry.ordinals[i - 1]))) {
      return discardInvalidCache(nullptr, "non-monotonic checkpoints");
    }
  }
  return true;
}
...
bool DictionaryStore::ensurePrepared(DictionaryEntry& entry, const std::function<void(int percent)>& onProgress) {
  ...
  while (pos < entry.idxFileSize) {
    ...
    if (!terminated || entry.idxFileSize - pos < 8) {
      idx.close();
      entry.checkpoints.clear();
      entry.ordinals.clear();
      entry.totalWords = 0;
      Storage.remove(entry.cachePath.c_str());
      LOG_ERR("DICT", "Invalid or truncated StarDict index: %s", entry.idxPath.c_str());
      return false;
    }
    ...
    pos += sizeof(skip);
    ++entry.totalWords;
    yieldDuringDictionaryScan(entry.totalWords);
