...
bool migrateMovedEpubState(const std::string& oldPath, const std::string& newPath, const std::string& oldCachePath,
  ...
  const std::string newCachePath = Epub::cachePathForFilePath(newPath, DUET_BOOKS_ROOT_PATH "");
  ...
  if (!oldCachePath.empty() && !LibraryInsights::registerMovedBookStatsAlias(oldCachePath, newCachePath)) {
    LOG_ERR("BookMove", "Failed to preserve stats identity for moved book %s -> %s", oldPath.c_str(),
            newPath.c_str());
    ok = false;
  }
  if (!Epub::migrateLayoutCacheForFileMove(oldPath, newPath, DUET_BOOKS_ROOT_PATH "")) {
    LOG_ERR("BookMove", "Failed to migrate disposable reader cache for moved book %s -> %s", oldPath.c_str(),
            newPath.c_str());
    ok = false;
  }
  ...
  if (!FAVORITES.updatePath(oldPath, newPath, oldCachePath, newCachePath)) {
    LOG_ERR("BookMove", "Failed to migrate favorite for moved book %s -> %s", oldPath.c_str(), newPath.c_str());
    ok = false;
  }
