...
std::string readSmallTextFile(const std::string& path) {
  ...
  HalFile file;
  if (!Storage.openFileForRead("IFF", path, file)) return "";

  std::string content;
  content.reserve(std::min(IfFoundFile::MAX_BYTES, file.fileSize()));
  uint8_t buffer[128];
  while (file.available() && content.size() < IfFoundFile::MAX_BYTES) {
    const size_t remaining = IfFoundFile::MAX_BYTES - content.size();
    const int read = file.read(buffer, std::min(remaining, sizeof(buffer)));
    if (read <= 0) break;
...
uint16_t readUtf16Unit(const std::string& value, const size_t offset, const bool littleEndian) {
  ...
  return littleEndian ? static_cast<uint16_t>(first | (second << 8))
