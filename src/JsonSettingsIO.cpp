...
bool JsonSettingsIO::saveSettings(const CrossPointSettings& s, const char* path) {
  ...
  if (s.sdFontFamilyName[0] != '\0') {
    ...
    if (s.sdFontPointSize != 0) {
      doc["sdFontPointSize"] = s.sdFontPointSize;
    }
  }
...
bool JsonSettingsIO::loadSettings(CrossPointSettings& s, const char* json, bool* needsResave) {
  ...
  if (doc["doublePwrBtn"].isNull() || doc["triplePwrBtn"].isNull()) {
    if (needsResave) *needsResave = true;
  }

  // New installs and settings files from before the long-press action existed
  // receive the bookmark shortcut. Any saved value, including Off, is an
  // explicit user choice and is preserved.
  if (doc["longPressMenuAction"].isNull()) {
    s.longPressMenuAction = CrossPointSettings::LONG_MENU_TOGGLE_BOOKMARK;
    if (needsResave) *needsResave = true;
  }
  ...
  s.sdFontPointSize = s.sdFontFamilyName[0] != '\0' ? (doc["sdFontPointSize"] | static_cast<uint8_t>(0)) : 0;
...
bool JsonSettingsIO::saveFavorites(const FavoritesStore& store, const char* path) {
  JsonDocument doc;
  JsonArray arr = doc["books"].to<JsonArray>();
  for (const auto& book : store.favoriteBooks) {
    JsonObject obj = arr.add<JsonObject>();
    obj["path"] = book.path;
    obj["title"] = book.title;
    obj["author"] = book.author;
...
bool JsonSettingsIO::loadFavorites(FavoritesStore& store, const char* json) {
  ...
  JsonArray arr = doc["books"].as<JsonArray>();
  store.favoriteBooks.reserve(std::min(arr.size(), FavoritesStore::MAX_FAVORITES));
  for (JsonObject obj : arr) {
    if (store.favoriteBooks.size() >= FavoritesStore::MAX_FAVORITES) break;
    FavoriteBook book;
    book.path = obj["path"] | std::string("");
    book.title = obj["title"] | std::string("");
    book.author = obj["author"] | std::string("");
    book.coverBmpPath = obj["coverBmpPath"] | std::string("");
    if (!book.path.empty()) store.favoriteBooks.push_back(std::move(book));
  }
  return true;
}
