...
constexpr char RECENT_BOOKS_FILE_BIN[] = DUET_STATE_ROOT_PATH "/recent.bin";
constexpr char RECENT_BOOKS_FILE_JSON[] = DUET_STATE_ROOT_PATH "/recent.json";
constexpr char RECENT_BOOKS_FILE_BAK[] = DUET_STATE_ROOT_PATH "/recent.bin.bak";
...
void RecentBooksStore::addOrUpdateBook(const std::string& path, const std::string& title, const std::string& author,
                                       const std::string& coverBmpPath) {
  // Reopening the current book is the dominant call (every book open). When the
  // entry is already front-most and identical, skip the pruneMissing() exists()
  // sweep and the full-file rewrite entirely.
...
bool RecentBooksStore::containsPath(const std::string& path) const {
  return std::any_of(recentBooks.begin(), recentBooks.end(), [&](const RecentBook& book) { return book.path == path; });
}
...
RecentBook RecentBooksStore::getDataFromBook(std::string path) const {
  ...
  if (FsHelpers::hasEpubExtension(lastBookFileName)) {
    ...
    // Handle XTC file
    Xtc xtc(path, DUET_BOOKS_ROOT_PATH "");
    if (xtc.load()) {
...
bool RecentBooksStore::loadFromFile() {
  // Try JSON first
  if (Storage.existsForRead(RECENT_BOOKS_FILE_JSON)) {
  ...
  if (Storage.existsForRead(RECENT_BOOKS_FILE_BIN)) {
