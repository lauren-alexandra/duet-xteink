...
constexpr char SETTINGS_FILE_BIN[] = DUET_STATE_ROOT_PATH "/settings.bin";
constexpr char SETTINGS_FILE_JSON[] = DUET_STATE_ROOT_PATH "/crossink-settings.json";
constexpr char LEGACY_SETTINGS_FILE_JSON[] = DUET_STATE_ROOT_PATH "/settings.json";
constexpr char SETTINGS_FILE_BAK[] = DUET_STATE_ROOT_PATH "/settings.bin.bak";
constexpr char LANG_FILE_BIN[] = DUET_STATE_ROOT_PATH "/language.bin";
constexpr char LANG_FILE_BAK[] = DUET_STATE_ROOT_PATH "/language.bin.bak";
...
const char* CrossPointSettings::getDefaultDeviceName() {
  if (gpio.deviceIsX3()) return "Duet X3";
  if (gpio.deviceIsX4()) return "Duet X4";
  return "Duet";
}
...
bool CrossPointSettings::loadFromFile() {
  ...
  auto loadJsonSettings = [this](const char* path, bool migrateToCurrentPath) -> JsonLoadStatus {
    if (!Storage.existsForRead(path)) return JsonLoadStatus::MissingOrEmpty;
  ...
  // Prefer Duet's compatibility-namespaced settings file. Use the old generic file only
  ...
  if (Storage.existsForRead(SETTINGS_FILE_BIN)) {
...
bool CrossPointSettings::migrateLanguageBinaryFile() {
  ...
  if (!Storage.existsForRead(LANG_FILE_BIN)) return false;
...
uint8_t CrossPointSettings::getSdFontTargetPointSize() const {
  if (sdFontPointSize != 0) return sdFontPointSize;
