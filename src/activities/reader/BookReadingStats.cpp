...
bool openStatsFileForRead(const std::string& cachePath, FsFile& f) {
  ...
  for (int version = STATS_FILE_VERSION - 1; version >= STATS_FILE_VERSION_V1; --version) {
    const std::string previousName = statsFileNameForVersion(static_cast<uint8_t>(version));
...
int32_t BookReadingStats::adjustReadingTime(const ReadingStatsDate& date, const int32_t requestedDelta) {
  return applyReadingTimeCorrection(totalReadingSeconds, timeOfDaySeconds, dayOfWeekSeconds, date, requestedDelta);
}
...
void BookReadingStats::save(const std::string& cachePath) const {
  ...
  writeLe32(data, 73, countedSessionSeconds);
  writeLe32(data, 77, latestSessionReadingSeconds);
  ...
  f.close();
  if (saved) {
    LibraryInsights::updateBookStatsIndex(cachePath, *this);
  } else {
    LOG_ERR("STATS", "Could not finish writing %s", statsFileName.c_str());
  }
}
...
bool BookReadingStats::remove(const std::string& cachePath) {
  ...
  for (uint8_t version = STATS_FILE_VERSION_V1; version < STATS_FILE_VERSION; ++version) {
    const std::string oldStatsFileName = statsFileNameForVersion(version);
    const std::string oldStatsPath = cachePath + "/" + oldStatsFileName;
    if (Storage.exists(oldStatsPath.c_str()) && !Storage.remove(oldStatsPath.c_str())) {
      LOG_ERR("STATS", "Could not delete %s", oldStatsFileName.c_str());
      ok = false;
    }
  }
  ...
  if (ok) {
    LibraryInsights::updateBookStatsIndex(cachePath, BookReadingStats{});
  } else {
