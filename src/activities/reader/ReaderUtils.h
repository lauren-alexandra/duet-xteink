...
inline PageTurnResult detectPageTurn(const MappedInputManager& input) {
  ...
  const bool shortPowerTurn = input.wasShortPowerActionResolved() &&
                              input.getResolvedShortPowerAction() == CrossPointSettings::SHORT_PWRBTN::PAGE_TURN;
...
inline bool getConfiguredReaderRefreshMode(HalDisplay::RefreshMode& mode) {
  switch (SETTINGS.readerRefreshMode) {
    case CrossPointSettings::READER_REFRESH_FAST:
      mode = HalDisplay::FAST_REFRESH;
      return true;
    case CrossPointSettings::READER_REFRESH_HALF:
      mode = HalDisplay::HALF_REFRESH;
      return true;
    case CrossPointSettings::READER_REFRESH_FULL:
      mode = HalDisplay::FULL_REFRESH;
      return true;
    case CrossPointSettings::READER_REFRESH_AUTO:
    default:
      return false;
  }
}
...
inline void displayWithRefreshCycle(const GfxRenderer& renderer, int& pagesUntilFullRefresh) {
  HalDisplay::RefreshMode configuredMode = HalDisplay::FAST_REFRESH;
...
inline bool displayConfiguredGrayscaleBase(const GfxRenderer& renderer, int& pagesUntilFullRefresh) {
  HalDisplay::RefreshMode configuredMode = HalDisplay::FAST_REFRESH;
  if (!getConfiguredReaderRefreshMode(configuredMode)) {
    return false;
  }
