...
void XtcReaderActivity::onEnter() {
  ...
  // Save current XTC as last opened book and add to recent books. The
  // last-active marker must move too — leaving it on the previous EPUB is one
  // of the stale-current-book (Yesteryear) mechanisms.
  APP_STATE.openEpubPath = xtc->getPath();
  APP_STATE.saveToFile();
  RECENT_BOOKS.addOrUpdateBook(xtc->getPath(), xtc->getTitle(), xtc->getAuthor(), xtc->getThumbBmpPath());
  CurrentBookStats::markLastActive(xtc->getPath());
...
void XtcReaderActivity::loop() {
  ...
  if (mappedInput.wasShortPowerActionResolved() &&
      executePowerAction(
          static_cast<CrossPointSettings::SHORT_PWRBTN>(mappedInput.getResolvedShortPowerAction()))) {
    return;
  }
  ...
  if (frontLongPressAction) {
    ...
    if (!frontButtonLongPressHandled && (prevLongPressed || nextLongPressed)) {
      ...
      if (prevLongPressed) {
        ...
        recordForwardPageTurn(forwardReadSeconds, shouldRecordForwardRead);
      }
      requestUpdate();
      return;
    }
...
bool XtcReaderActivity::currentPageReadingSecondsForStats(uint32_t& seconds, const char* source) const {
  seconds = 0;
  if (!xtc || !shouldTrackReadingStatsForPath(xtc->getPath()) || pageShownAtMs == 0UL) {
    return false;
  }
...
void XtcReaderActivity::recordForwardPageTurn(const uint32_t seconds, const bool qualifiesForStats) {
  (void)seconds;
  if (sessionScreenPages < UINT16_MAX) {
    sessionScreenPages++;
  }
  if (qualifiesForStats) {
...
void XtcReaderActivity::commitReadingStats() {
  if (readingStatsCommitted || !xtc || !shouldTrackReadingStatsForPath(xtc->getPath())) {
    return;
  }
  ...
  if (hasSessionStartLocalDateTime && (elapsedSecs > 0 || qualifiesAsSession)) {
    ...
    ReadingLedger::recordReadingSpan(sessionStartLocalDateTime, elapsedSecs, sessionScreenPages,
                                     xtc->getCachePath(), xtc->getTitle());
    SessionLog::append(sessionStartLocalDateTime, elapsedSecs, sessionScreenPages, xtc->getCachePath(),
                       xtc->getTitle());
  }
}

void XtcReaderActivity::goHomeAfterReading() {
  if (SETTINGS.showStatsAfterReading == 0 || !xtc || !shouldTrackReadingStatsForPath(xtc->getPath())) {
    onGoHome();
    return;
  }

  commitReadingStats();
  if (!xtc || (sessionReadingSeconds == 0 && sessionScreenPages == 0)) {
    onGoHome();
    return;
  }

  const bool hasSyncedStats = GlobalReadingStats::hasSyncedStats();
  pauseReadingStatsTimer("post_reading_stats");
  if (hasSyncedStats) {
    startActivityForResult(
        std::make_unique<BookStatsActivity>(renderer, mappedInput, xtc->getTitle(), xtc->getCachePath(), stats,
                                            getCurrentBookProgressPercent(), false, 0, globalStats,
                                            GlobalReadingStats::loadAggregated(globalStats), true),
        [this](const ActivityResult&) { onGoHome(); });
  } else {
    startActivityForResult(
        std::make_unique<BookStatsActivity>(renderer, mappedInput, xtc->getTitle(), xtc->getCachePath(), stats,
                                            getCurrentBookProgressPercent(), false, 0, globalStats, true),
        [this](const ActivityResult&) { onGoHome(); });
  }
}
...
void XtcReaderActivity::setBookCompleted(const bool isCompleted) {
  ...
  const bool includeInAggregateStats = shouldTrackReadingStatsForPath(xtc->getPath());
...
void XtcReaderActivity::openReadingStats() {
  ...
  if (shouldTrackReadingStatsForPath(xtc->getPath())) {
  ...
  if (hasSyncedStats) {
    startActivityForResult(
        std::make_unique<BookStatsActivity>(renderer, mappedInput, xtc->getTitle(), xtc->getCachePath(), displayStats,
                                            getCurrentBookProgressPercent(), false, 0, globalStats,
                                            displayAllDevicesStats, false, sessionSnapshot),
    ...
    startActivityForResult(std::make_unique<BookStatsActivity>(
                               ...
                               getCurrentBookProgressPercent(), false, 0, globalStats, false, sessionSnapshot),
                           [this](const ActivityResult&) {
                             if (xtc) {
                               stats = BookReadingStats::load(xtc->getCachePath());
                             }
...
bool XtcReaderActivity::drawCurrentPageToBuffer(const std::string& filePath, GfxRenderer& renderer) {
  Xtc xtc(filePath, DUET_BOOKS_ROOT_PATH "");
  if (!xtc.load()) {
