...
bool shouldIgnoreReadingStatsPath(const std::string& path) {
...
bool shouldTrackReadingStatsForPath(const std::string& path) {
  return SETTINGS.shouldTrackReadingStats() && !shouldIgnoreReadingStatsPath(path);
}
...
bool getCurrentLocalReadingStatsDateTime(ReadingStatsDateTime& outDateTime) {
#ifdef SIMULATOR
  const char* simulated = std::getenv("CROSSINK_SIMULATOR_LOCAL_DATETIME");
  if (simulated != nullptr && simulated[0] != '\0') {
    unsigned year = 0;
    unsigned month = 0;
    unsigned day = 0;
    unsigned hour = 0;
    unsigned minute = 0;
    if (std::sscanf(simulated, "%u-%u-%uT%u:%u", &year, &month, &day, &hour, &minute) == 5) {
      outDateTime.date = {static_cast<uint16_t>(year), static_cast<uint8_t>(month), static_cast<uint8_t>(day)};
      outDateTime.hour = static_cast<uint8_t>(hour);
      outDateTime.minute = static_cast<uint8_t>(minute);
      outDateTime.second = 0;
      if (outDateTime.isValid()) return true;
    }
    outDateTime = {};
    return false;
  }
  ...
  if (!halClock.getDateTime(year, month, day, hour, minute)) {
    return getClocklessReadingStatsDateTime(outDateTime);
  }
...
int32_t applyReadingTimeCorrection(uint32_t& totalReadingSeconds,
                                   std::array<uint32_t, READING_TIME_BUCKET_COUNT>& timeOfDaySeconds,
                                   std::array<uint32_t, READING_DAY_OF_WEEK_COUNT>& dayOfWeekSeconds,
                                   const ReadingStatsDate& date, const int32_t requestedDelta) {
  if (!date.isValid() || requestedDelta == 0) return 0;

  if (requestedDelta > 0) {
    const uint32_t requested = static_cast<uint32_t>(requestedDelta);
    const uint32_t applied = std::min(requested, UINT32_MAX - totalReadingSeconds);
    totalReadingSeconds += applied;
    const size_t afternoon = static_cast<size_t>(ReadingTimeBucket::Afternoon);
    timeOfDaySeconds[afternoon] =
        applied > UINT32_MAX - timeOfDaySeconds[afternoon] ? UINT32_MAX : timeOfDaySeconds[afternoon] + applied;
    const size_t weekDay = readingStatsDayOfWeekIndex(date);
    dayOfWeekSeconds[weekDay] =
        applied > UINT32_MAX - dayOfWeekSeconds[weekDay] ? UINT32_MAX : dayOfWeekSeconds[weekDay] + applied;
    return static_cast<int32_t>(applied);
  }

  uint32_t remaining = std::min<uint32_t>(static_cast<uint32_t>(-static_cast<int64_t>(requestedDelta)),
                                          totalReadingSeconds);
  const uint32_t applied = remaining;
  totalReadingSeconds -= applied;

  const size_t preferredTime = static_cast<size_t>(ReadingTimeBucket::Afternoon);
  const uint32_t fromPreferredTime = std::min(timeOfDaySeconds[preferredTime], remaining);
  timeOfDaySeconds[preferredTime] -= fromPreferredTime;
  remaining -= fromPreferredTime;
  for (size_t i = 0; i < timeOfDaySeconds.size() && remaining > 0; ++i) {
    if (i == preferredTime) continue;
    const uint32_t amount = std::min(timeOfDaySeconds[i], remaining);
    timeOfDaySeconds[i] -= amount;
    remaining -= amount;
  }

  remaining = applied;
  const size_t preferredDay = readingStatsDayOfWeekIndex(date);
  const uint32_t fromPreferredDay = std::min(dayOfWeekSeconds[preferredDay], remaining);
  dayOfWeekSeconds[preferredDay] -= fromPreferredDay;
  remaining -= fromPreferredDay;
  for (size_t i = 0; i < dayOfWeekSeconds.size() && remaining > 0; ++i) {
    if (i == preferredDay) continue;
    const uint32_t amount = std::min(dayOfWeekSeconds[i], remaining);
    dayOfWeekSeconds[i] -= amount;
    remaining -= amount;
  }
  return -static_cast<int32_t>(applied);
}
...
void clearReadingHistoryDay(const uint32_t anchorDay, std::array<uint8_t, READING_HISTORY_BYTES>& bits,
                            const uint32_t dayIndex) {
  if (dayIndex > anchorDay) return;
  const uint32_t delta = anchorDay - dayIndex;
  if (delta >= READING_HISTORY_DAYS) return;
  bits[delta / 8u] &= static_cast<uint8_t>(~(1u << (delta % 8u)));
}
