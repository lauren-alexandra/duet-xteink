...
void KOReaderSyncActivity::ensureEpubLoaded() {
  if (!epub) {
    ...
    epub = std::make_shared<Epub>(epubPath, DUET_BOOKS_ROOT_PATH "");
    epub->setupCacheDir();
...
void KOReaderSyncActivity::performSync() {
...

  {
    ...
    constexpr float kSameProgressTolerance = 0.0005f;
    const float difference = localProgress.percentage - remoteProgress.percentage;
    if (difference > kSameProgressTolerance) {
      furthestProgressAction = FurthestProgressAction::UploadLocal;
    } else if (difference < -kSameProgressTolerance) {
      furthestProgressAction = FurthestProgressAction::ApplyRemote;
    } else {
      furthestProgressAction = FurthestProgressAction::AlreadySynced;
    }
  }
  requestUpdate(true);
}
...
void KOReaderSyncActivity::render(RenderLock&&) {
  ...
  if (state == SHOWING_RESULT) {
    ...
    const int optionY = top + 232;
    const int optionHeight = 34;
    renderer.fillRect(screen.x, optionY - 3, screen.width - 1, optionHeight);
    renderer.drawCenteredText(UI_10_FONT_ID, optionY + 1, "Sync to Furthest", false, EpdFontFamily::BOLD);

    const char* actionDescription = "Both devices are already at this position";
    if (furthestProgressAction == FurthestProgressAction::ApplyRemote) {
      actionDescription = "Use the farther remote position on this device";
    } else if (furthestProgressAction == FurthestProgressAction::UploadLocal) {
      actionDescription = "Share this device's farther position";
    }
    const auto actionLines =
        renderer.wrappedText(SMALL_FONT_ID, actionDescription, screen.width - metrics.contentSidePadding * 2, 2);
    int actionY = optionY + optionHeight + 5;
    for (const auto& line : actionLines) {
      renderer.drawCenteredText(SMALL_FONT_ID, actionY, line.c_str(), true);
      actionY += renderer.getLineHeight(SMALL_FONT_ID);
    }

    // Bottom button hints
    const auto labels = mappedInput.mapLabels(tr(STR_BACK), "Sync", "", "");
    GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4, true);
    renderer.displayBuffer();
    return;
  }
