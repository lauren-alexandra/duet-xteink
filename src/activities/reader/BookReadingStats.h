...
struct BookReadingStats {
  ...
  uint16_t sessionCount = 0;              // Qualifying sessions with at least one forward page turn
  uint32_t totalReadingSeconds = 0;       // Accumulated reading time in seconds
  uint32_t countedSessionSeconds = 0;     // Reading time belonging to qualifying sessions
  ...
  uint32_t latestSessionReadingSeconds = 0;
  ...
  bool hasReliableTimeLeftBasis() const {
    return totalReadingSeconds >= MIN_RELIABLE_TIME_LEFT_READING_SECONDS && avgSecondsPerForwardPage > 0 &&
           paceSampleCount >= MIN_RELIABLE_TIME_LEFT_PACE_SAMPLES;
  }

  // Loads stats from cachePath/stats_v7.bin, with fallback reads from the
  ...
  // Saves stats to cachePath/stats_v7.bin.
  void save(const std::string& cachePath) const;

  // Deletes versioned stats files and the legacy
  ...
  // Applies a manual total-time correction and keeps the aggregate day/time
  // buckets internally consistent. Returns the delta that was actually applied.
