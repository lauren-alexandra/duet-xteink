...
//
// Binary layout v4 (163 bytes):
//   [0-158]   v3 fields
//   [159-162] countedSessionSeconds     uint32_t LE
...
static constexpr char GLOBAL_STATS_PATH[] = DUET_STATE_ROOT_PATH "/global_stats.bin";
static constexpr char GLOBAL_STATS_BAK_PATH[] = DUET_STATE_ROOT_PATH "/global_stats.bin.bak";
static constexpr char SYNCED_STATS_DIR[] = DUET_STATE_ROOT_PATH "/synced_stats";
...
void loadCommonFields(const uint8_t* data, GlobalReadingStats& out) {
  ...
  out.countedSessionSeconds = out.totalReadingSeconds;
...
void serializeStats(const GlobalReadingStats& stats, uint8_t* data) {
  ...
  data[159] = stats.countedSessionSeconds & 0xFF;
  data[160] = (stats.countedSessionSeconds >> 8) & 0xFF;
  data[161] = (stats.countedSessionSeconds >> 16) & 0xFF;
  data[162] = (stats.countedSessionSeconds >> 24) & 0xFF;
}
...
StatsLoadOutcome loadFromOpenFile(FsFile& f, GlobalReadingStats& out) {
  ...
  if ((n == GLOBAL_STATS_FILE_SIZE && data[0] != GLOBAL_STATS_VERSION) ||
      (n == GLOBAL_STATS_FILE_SIZE_V3 && data[0] != GLOBAL_STATS_VERSION_V3)) {
    return outcome;
  }
  loadCommonFields(data, out);
  out.completedBooks = readLe32(data, 13);
  ...
  out.countedSessionSeconds = n == GLOBAL_STATS_FILE_SIZE ? readLe32(data, 159) : out.totalReadingSeconds;
  outcome.result = StatsLoadResult::Ok;
  return outcome;
}
...
bool GlobalReadingStats::hasSyncedStats() {
  ...
  for (FsFile file = dir.openNextFile(); file; file = dir.openNextFile()) {
    ...
    const bool possiblePeerStats =
        !isDirectory && nameLen > 0 && (localFileName.empty() || strcmp(name, localFileName.c_str()) != 0);
    if (possiblePeerStats) {
...
uint8_t GlobalReadingStats::loadSyncedPeers(GlobalReadingStats* out, std::string* names, const uint8_t maxCount) {
  ...
  const std::string localFileName = localSyncedStatsFileName();
  uint8_t count = 0;
  for (FsFile file = dir.openNextFile(); file && count < maxCount; file = dir.openNextFile()) {
    const bool isDirectory = file.isDirectory();
    const size_t nameLen = file.getName(name, sizeof(name));
    if (!isDirectory && nameLen > 0 && (localFileName.empty() || strcmp(name, localFileName.c_str()) != 0)) {
      GlobalReadingStats peer;
      const StatsLoadOutcome outcome = loadFromOpenFile(file, peer);
      if (outcome.result == StatsLoadResult::Ok) {
        if (names != nullptr) {
          std::string txtName = name;
          const size_t dot = txtName.rfind(".bin");
          if (dot != std::string::npos) txtName.replace(dot, 4, ".txt");
          const std::string namePath = std::string(DUET_STATE_ROOT_PATH "/synced_names/") + txtName;
          names[count].clear();
          if (Storage.exists(namePath.c_str())) {
            String saved = Storage.readFile(namePath.c_str());
            saved.trim();
            names[count] = saved.c_str();
          }
        }
        out[count++] = peer;
      }
...
int32_t GlobalReadingStats::adjustReadingTime(const ReadingStatsDate& date, const int32_t requestedDelta) {
  const int32_t applied =
      applyReadingTimeCorrection(totalReadingSeconds, timeOfDaySeconds, dayOfWeekSeconds, date, requestedDelta);
  if (applied > 0) {
    markReadingHistoryDay(readingHistoryAnchorDay, readingHistoryBits, readingStatsDayIndex(date));
    longestReadingStreak = std::max(longestReadingStreak,
...
bool GlobalReadingStats::hasReadingOnDay(const uint32_t dayIndex) const {
  if (readingHistoryAnchorDay == 0 && (readingHistoryBits[0] & 1u) == 0) return false;
  if (dayIndex > readingHistoryAnchorDay) return false;
  const uint32_t delta = readingHistoryAnchorDay - dayIndex;
  if (delta >= READING_HISTORY_DAYS) return false;
  return (readingHistoryBits[delta / 8u] & static_cast<uint8_t>(1u << (delta % 8u))) != 0;
}
