 ...
 * 4. Show the local and remote positions with the furthest one identified
 * 5. Apply or upload that furthest position only after user confirmation
 */
class KOReaderSyncActivity final : public Activity {
 ...
 private:
  ...
  enum class FurthestProgressAction : uint8_t {
    AlreadySynced,
    ApplyRemote,
    UploadLocal,
  };
  FurthestProgressAction furthestProgressAction = FurthestProgressAction::AlreadySynced;
