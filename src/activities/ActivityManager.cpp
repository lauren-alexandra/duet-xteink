...
void ActivityManager::goToReader(std::string path, const bool suppressBackRelease) {
  // Update the in-memory active book immediately. Persistence is deferred until
  // the reader's first page paints so this small SD write cannot delay opening.
...
void ActivityManager::goToBoot(const bool forcePanelScrub) {
  replaceActivity(std::make_unique<BootActivity>(renderer, mappedInput, forcePanelScrub));
}
...
RequestUpdateResult ActivityManager::requestUpdateAndWait() {
  ...
  xTaskNotify(renderTaskHandle, 1, eIncrement);
  // Bounded: an unbounded wait here froze the whole main loop (Back included)
  // when the render task failed to signal back. Any legitimate render, X3
  // resync passes included, completes well inside this window; on timeout we
  // deregister and proceed unpainted rather than deadlock.
#ifndef pdMS_TO_TICKS
  // Simulator FreeRTOS shim: ticks are milliseconds.
#define pdMS_TO_TICKS(ms) (ms)
#endif
  if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(8000)) == 0) {
    taskENTER_CRITICAL(&renderStateMux);
    if (waitingTaskHandle == xTaskGetCurrentTaskHandle()) {
      waitingTaskHandle = nullptr;
    }
    taskEXIT_CRITICAL(&renderStateMux);
    LOG_ERR("ACT", "requestUpdateAndWait() timed out; continuing without confirmed paint");
    return RequestUpdateResult::Rejected;
  }
...
RenderLock::RenderLock(const AcquireMode mode) {
  isLocked = xSemaphoreTake(activityManager.renderingMutex, mode == AcquireMode::Wait ? portMAX_DELAY : 0) == pdTRUE;
}
