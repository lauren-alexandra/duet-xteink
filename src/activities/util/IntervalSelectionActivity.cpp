...
void IntervalSelectionActivity::render(RenderLock&&) {
  ...
  const bool isLineSpacing = titleId == StrId::STR_LINE_SPACING;
  if (isLineSpacing) {
    const char* spacingLabel = value <= 95 ? tr(STR_TIGHT) : (value >= 110 ? tr(STR_WIDE) : tr(STR_NORMAL));
    renderer.drawCenteredText(UI_10_FONT_ID, 116, spacingLabel, true);
  }
  ...
  if (isLineSpacing) {
    const int previewWidth = std::min(300, std::max(220, screenWidth - 40));
    constexpr int previewHeight = 126;
    constexpr int previewTop = 238;
    constexpr int previewPadding = 14;
    const int previewX = std::max(0, (screenWidth - previewWidth) / 2);
    renderer.drawRect(previewX, previewTop, previewWidth, previewHeight);

    const int textWidth = previewWidth - previewPadding * 2;
    const auto previewLines =
        renderer.wrappedText(UI_10_FONT_ID, tr(STR_FONT_PREVIEW_TEXT), textWidth, 4, EpdFontFamily::REGULAR);
    const int glyphHeight = renderer.getLineHeight(UI_10_FONT_ID);
    const int lineStep = std::max(1, (glyphHeight * value + 50) / 100);
    const int blockHeight =
        previewLines.empty() ? glyphHeight : glyphHeight + lineStep * (static_cast<int>(previewLines.size()) - 1);
    int lineY = previewTop + std::max(4, (previewHeight - blockHeight) / 2);
    for (const auto& line : previewLines) {
      renderer.drawText(UI_10_FONT_ID, previewX + previewPadding, lineY, line.c_str(), true);
      lineY += lineStep;
    }
  }
