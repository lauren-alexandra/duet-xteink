...
enum class InputType { Text, Password, Url, Search };
...
using KeyboardSuggestionProvider =
    std::function<std::vector<KeyboardSuggestion>(const std::string& text, size_t maxSuggestions)>;
...
class KeyboardEntryActivity : public Activity {
 public:
  explicit KeyboardEntryActivity(GfxRenderer& renderer, MappedInputManager& mappedInput,
                                 ...
                                 KeyboardSuggestionProvider suggestionProvider = {})
      : Activity("KeyboardEntry", renderer, mappedInput),
        ...
        inputType(inputType),
        minLength(minLength),
        suggestionProvider(std::move(suggestionProvider)) {}
...
#ifdef SIMULATOR
  const std::string& simulatorText() const { return text; }
#endif
 ...
 private:
  ...
  unsigned long suggestionsDueAt = 0;
  ...
  static constexpr uint16_t SEARCH_SUGGESTION_DELAY_MS = 120;
