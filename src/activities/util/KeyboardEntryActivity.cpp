...
char KeyboardEntryActivity::getAlternativeChar() const {
  if (symMode || urlMode || inputType == InputType::Search) return '\0';
...
void KeyboardEntryActivity::refreshSuggestions() {
  completionApplied = false;
  suggestionsPending = false;
  suggestionsDueAt = 0;
  selectedSuggestion = 0;
  suggestions.clear();
  if (inputType != InputType::Search || !suggestionProvider || text.length() < 2) return;

  suggestions = suggestionProvider(text, MAX_SUGGESTIONS);
  if (suggestions.size() > MAX_SUGGESTIONS) suggestions.resize(MAX_SUGGESTIONS);
}

void KeyboardEntryActivity::scheduleSuggestionRefresh() {
  completionApplied = false;
  selectedSuggestion = 0;
  suggestions.clear();
  suggestionsPending = inputType == InputType::Search && suggestionProvider && text.length() >= 2;
  suggestionsDueAt = suggestionsPending ? millis() + SEARCH_SUGGESTION_DELAY_MS : 0;
}

void KeyboardEntryActivity::selectNextSuggestion() {
  if (suggestions.empty()) return;
  selectedSuggestion = (selectedSuggestion + 1) % suggestions.size();
}

void KeyboardEntryActivity::applySelectedSuggestion() {
  if (suggestions.empty() || selectedSuggestion >= suggestions.size()) return;
  text = suggestions[selectedSuggestion].value;
  if (maxLength != 0 && text.length() > maxLength) text.resize(maxLength);
  cursorPos = text.length();
  suggestions.clear();
  selectedSuggestion = 0;
  completionApplied = true;
  selectedRow = getTotalRowCount() - 1;
  selectedCol = static_cast<int>(SpecialKeyType::Ok);
}
...
void KeyboardEntryActivity::loop() {
  ...
  if (suggestionsPending && static_cast<long>(millis() - suggestionsDueAt) >= 0) {
    refreshSuggestions();
    requestUpdate();
  }
...
void KeyboardEntryActivity::render(RenderLock&&) {
  ...
  const bool searchAutocomplete = inputType == InputType::Search && suggestionProvider;
  if (searchAutocomplete) {
    const int suggestionTop = underlineBottom + 2;
    const int suggestionBottom = keyboardStartY - 4;
    const int availableHeight = std::max(0, suggestionBottom - suggestionTop);
    if (!suggestions.empty() && availableHeight > 0) {
      const int suggestionLineHeight = renderer.getLineHeight(SMALL_FONT_ID);
      const int rowHeight = suggestionLineHeight + 5;
      const int visibleRows = std::min<int>(suggestions.size(), std::max(1, availableHeight / rowHeight));
      const int rowsHeight = visibleRows * rowHeight;
      int rowY = suggestionTop + std::max(0, (availableHeight - rowsHeight) / 2);
      const int rowX = effectiveMargin;
      const int rowWidth = pageWidth - 2 * effectiveMargin;
      for (int i = 0; i < visibleRows; ++i) {
        const bool selected = static_cast<size_t>(i) == selectedSuggestion;
        std::string label = suggestions[i].category;
        if (!label.empty()) label += ": ";
        label += suggestions[i].value;
        label = renderer.truncatedText(SMALL_FONT_ID, label.c_str(), rowWidth - 12);
        if (selected) renderer.fillRect(rowX, rowY, rowWidth, rowHeight - 1, true);
        renderer.drawText(SMALL_FONT_ID, rowX + 6, rowY + 2, label.c_str(), !selected);
        rowY += rowHeight;
      }
    } else {
      const char* message = suggestionsPending ? tr(STR_KB_FINDING_SUGGESTIONS)
                            : completionApplied ? tr(STR_KB_COMPLETION_READY)
                            : text.length() < 2 ? tr(STR_KB_TYPE_TWO_FOR_SUGGESTIONS)
                                              : tr(STR_KB_NO_COMPLETIONS);
      const int messageY = suggestionTop + std::max(0, availableHeight - tipsLh) / 2;
      renderer.drawCenteredText(SMALL_FONT_ID, messageY, message, true);
    }
  }
  ...
  const BottomKeyInfo bottomKeys[BOTTOM_KEY_COUNT] = {
      {inputType == InputType::Search
           ? (suggestions.empty() ? KeyboardKeyType::Disabled : KeyboardKeyType::Mode)
           : ((symMode || urlMode) ? KeyboardKeyType::Disabled : KeyboardKeyType::Shift),
       inputType == InputType::Search ? "Next"
                                      : ((symMode || urlMode) ? shiftString[0] : shiftString[shiftState])},
      {inputType == InputType::Search
           ? (suggestions.empty() ? KeyboardKeyType::Disabled : KeyboardKeyType::Mode)
           : KeyboardKeyType::Mode,
       inputType == InputType::Search ? "Fill" : (urlMode ? "abc" : (symMode ? "abc" : "#@!"))},
