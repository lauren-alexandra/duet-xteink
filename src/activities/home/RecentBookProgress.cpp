...
float loadEpubProgressPercent(const RecentBook& book) {
  const std::string cachePath = Epub::cachePathForFilePath(book.path, DUET_BOOKS_ROOT_PATH "");
  float progressPercent = -1.0f;
  if (EpubReaderUtils::loadProgressPercent(cachePath, progressPercent, "RBPR")) return progressPercent;

  EpubReaderUtils::Progress progress;
  if (!EpubReaderUtils::loadProgress(cachePath, progress, "RBPR") || !progress.hasPageCount) {
    return -1.0f;
  }
  ...
  float sizeProgress = 0.0f;
  if (!BookMetadataCache::readSizeProgress(cachePath, progress.spineIndex, chapterProgress, sizeProgress)) {
    return -1.0f;
  }

  progressPercent = clampProgressPercent(sizeProgress * 100.0f);
  EpubReaderUtils::saveProgressPercent(cachePath, progressPercent, "RBPR");
...
float loadTxtProgressPercent(const RecentBook& book) {
  Txt txt(book.path, DUET_BOOKS_ROOT_PATH "");
  if (!txt.load()) {
    return -1.0f;
  }
