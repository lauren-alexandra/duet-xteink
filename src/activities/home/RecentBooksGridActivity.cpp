...
void RecentBooksGridActivity::onEnter() {
  ...
  loadRecentBooks();
  selectorIndex = 0;
  paintedSelectorIndex.store(NO_PAGE_LOADED, std::memory_order_relaxed);
  paintedPageStart.store(NO_PAGE_LOADED, std::memory_order_relaxed);
...
void RecentBooksGridActivity::requestUpdate(const bool immediate) {
  pendingGridRefresh.store(static_cast<uint8_t>(GridRefresh::Full), std::memory_order_release);
  Activity::requestUpdate(immediate);
}

void RecentBooksGridActivity::requestFastGridFocus() {
  pendingGridRefresh.store(static_cast<uint8_t>(GridRefresh::Focus), std::memory_order_release);
...
void RecentBooksGridActivity::loop() {
  ...
  auto handleNav = [this, listSize](NavDirection direction) {
    ...
    ensureProgressLoaded(selectorIndex);
    const int previousPageStart = (previousIndex / BOOKS_PER_PAGE) * BOOKS_PER_PAGE;
    const int nextPageStart = (selectorIndex / BOOKS_PER_PAGE) * BOOKS_PER_PAGE;
    if (previousPageStart == nextPageStart && paintedPageStart.load(std::memory_order_acquire) == nextPageStart) {
      requestFastGridFocus();
    } else {
      requestUpdate();
    }
  };

  // Move on the physical press, not after release. The cover browser already
  // uses this contract and it makes a short tap feel immediate on both panels.
  buttonNavigator.onPress({MappedInputManager::Button::Right}, [&] { handleNav(NavDirection::Right); });
  buttonNavigator.onPress({MappedInputManager::Button::Left}, [&] { handleNav(NavDirection::Left); });
  buttonNavigator.onPress({MappedInputManager::Button::Down}, [&] { handleNav(NavDirection::Down); });
  buttonNavigator.onPress({MappedInputManager::Button::Up}, [&] { handleNav(NavDirection::Up); });
      ...
      [this, book](const ActivityResult& result) {
        ...
        switch (static_cast<FileBrowserAction>(actionResult->action)) {
          ...
          case FileBrowserAction::RemoveBookFavorite: {
            bool favorite = false;
            if (BookActions::toggleBookFavorite(book.path, favorite)) {
              BookActions::drawToast(renderer, favorite ? tr(STR_ADDED_TO_FAVORITES) : tr(STR_REMOVED_FROM_FAVORITES));
              delay(700);
            }
            reloadAfterBookAction();
            return;
...
void RecentBooksGridActivity::drawSelectedBookTitle(const int startXOffset, const int totalGridWidth,
                                                    const int contentTop, const int titleStripHeight,
                                                    const int titleGridGap) {
  ...
  renderer.fillRect(0, headerBottomY, renderer.getScreenWidth(), std::max(1, gridTopY - headerBottomY), false);
  ...
  const int progressSuffixWidth = hasProgress ? separatorWidth + progressWidth + progressIconGap + progressIconSize : 0;
  ...
  int progressX = std::min(startXOffset + titleWidth, startXOffset + totalGridWidth - progressSuffixWidth);
...
bool RecentBooksGridActivity::renderFastGridFocus() {
  if (recentBooks.empty() || selectorIndex < 0 || selectorIndex >= static_cast<int>(recentBooks.size())) {
    return false;
  }

  const int pageStart = (selectorIndex / BOOKS_PER_PAGE) * BOOKS_PER_PAGE;
  const int previousIndex = paintedSelectorIndex.load(std::memory_order_acquire);
  if (paintedPageStart.load(std::memory_order_acquire) != pageStart || previousIndex < pageStart ||
      previousIndex >= std::min(pageStart + BOOKS_PER_PAGE, static_cast<int>(recentBooks.size()))) {
    return false;
  }
  if (previousIndex == selectorIndex) return true;
  ...
  const auto cellPosition = [&](const int index, int& x, int& y) {
    const int indexInPage = index - pageStart;
    x = startXOffset + (indexInPage % kGridColumns) * (COVER_WIDTH + gridSpacing);
    y = contentTop + titleStripHeight + titleGridGap + (indexInPage / kGridColumns) * (COVER_HEIGHT + rowSpacing);
  };
  int previousX = 0;
  int previousY = 0;
  int nextX = 0;
  int nextY = 0;
  cellPosition(previousIndex, previousX, previousY);
  cellPosition(selectorIndex, nextX, nextY);

  // Erase only the old ring. The cover itself remains untouched in the frame
  // buffer, so a focus move never reopens or decodes a BMP from the SD card.
  const int ringWidth = COVER_WIDTH + selectionOuterInset * 2;
  renderer.fillRect(previousX - selectionOuterInset, previousY - selectionOuterInset, ringWidth, selectionOuterInset,
                    false);
  renderer.fillRect(previousX - selectionOuterInset, previousY + COVER_HEIGHT, ringWidth, selectionOuterInset, false);
  renderer.fillRect(previousX - selectionOuterInset, previousY, selectionOuterInset, COVER_HEIGHT, false);
  renderer.fillRect(previousX + COVER_WIDTH, previousY, selectionOuterInset, COVER_HEIGHT, false);

  renderer.drawRoundedRect(nextX - selectionPadding, nextY - selectionPadding, COVER_WIDTH + selectionPadding * 2,
                           COVER_HEIGHT + selectionPadding * 2, 3, kCoverCornerRadius + selectionPadding, true);
  renderer.drawRoundedRect(nextX - selectionOuterInset, nextY - selectionOuterInset,
                           COVER_WIDTH + selectionOuterInset * 2, COVER_HEIGHT + selectionOuterInset * 2, 1,
                           kCoverCornerRadius + selectionOuterInset, true);
  drawSelectedBookTitle(startXOffset, totalGridWidth, contentTop, titleStripHeight, titleGridGap);

  const int ringX = std::min(previousX, nextX) - selectionOuterInset;
  const int ringY = std::min(previousY, nextY) - selectionOuterInset;
  const int ringRight = std::max(previousX, nextX) + COVER_WIDTH + selectionOuterInset;
  const int ringBottom = std::max(previousY, nextY) + COVER_HEIGHT + selectionOuterInset;
  renderer.displayWindow(ringX, ringY, ringRight - ringX, ringBottom - ringY, false, false);
  const int headerBottomY = CompactHeader::headerBottomY(metrics);
  const int gridTopY = contentTop + titleStripHeight + titleGridGap;
  renderer.displayWindow(0, headerBottomY, pageWidth, std::max(1, gridTopY - headerBottomY), false, false);
  paintedSelectorIndex.store(selectorIndex, std::memory_order_release);
  paintedPageStart.store(pageStart, std::memory_order_release);
  return true;
}

void RecentBooksGridActivity::render(RenderLock&&) {
  const auto refresh = static_cast<GridRefresh>(
      pendingGridRefresh.exchange(static_cast<uint8_t>(GridRefresh::None), std::memory_order_acq_rel));
  if (refresh == GridRefresh::Focus && renderFastGridFocus()) return;
  ...
  if (recentBooks.empty()) {
    ...
    for (int i = 0; i < pageCount; ++i) {
      ...
      if (!thumbPath.empty() && Storage.existsForRead(thumbPath)) {
        FsFile file;
  ...
  // Keep the panel awake while browsing covers. More importantly, do not run
  // any thumbnail generation after this frame: the card is already prebuilt,
  // and a missing cover must never block navigation through Recent Books.
  renderer.displayBuffer(HalDisplay::FAST_REFRESH, false, false);
  paintedSelectorIndex.store(recentBooks.empty() ? NO_PAGE_LOADED : selectorIndex, std::memory_order_release);
  paintedPageStart.store(recentBooks.empty() ? NO_PAGE_LOADED : pageStart, std::memory_order_release);
}
