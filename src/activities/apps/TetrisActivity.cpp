...
void TetrisActivity::render(RenderLock&&) {
  ...
  // Keep play responsive on e-ink, with a periodic clean waveform to clear
  // accumulated ghosting from the moving blocks.
  const bool cleanRefresh = (refreshCount++ % 16) == 0 || state == GAME_OVER;
  renderer.displayBuffer(cleanRefresh ? HalDisplay::HALF_REFRESH : HalDisplay::FAST_REFRESH);
}
...
void TetrisActivity::renderPlaying() const {
  ...
  panelY += 16;
  const int previewCellSize = std::max(6, std::min(cellSize, (screenW - panelX - 14) / 4));
  renderer.drawRect(panelX, panelY, previewCellSize * 4 + 4, previewCellSize * 4 + 4);
  uint16_t nextShape = PIECES[nextPiece].shape[0];
  for (int r = 0; r < 4; r++) {
    for (int c = 0; c < 4; c++) {
      if (!getPieceBit(nextShape, r, c)) continue;
      int sx = panelX + 2 + c * previewCellSize;
      int sy = panelY + 2 + r * previewCellSize;
      renderer.fillRect(sx, sy, previewCellSize, previewCellSize, true);
      renderer.drawRect(sx + 1, sy + 1, previewCellSize - 2, previewCellSize - 2, false);
    }
  }
}
