...
void DictionaryActivity::render(RenderLock&&) {
  ...
  CompactHeader::drawTitle(renderer, tr(STR_DICTIONARY), true);
  ...
  if (!activeLabel.empty()) {
    ...
    const std::string label = renderer.truncatedText(SMALL_FONT_ID, activeLabel.c_str(), maxWidth);
    const int labelWidth = renderer.getTextWidth(SMALL_FONT_ID, label.c_str());
    renderer.drawText(SMALL_FONT_ID, pageWidth - metrics.contentSidePadding - labelWidth,
                      metrics.topPadding + metrics.headerHeight - renderer.getLineHeight(SMALL_FONT_ID) - 2,
                      label.c_str());
  }
