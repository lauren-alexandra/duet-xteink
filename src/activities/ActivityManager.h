...
class ActivityManager {
 ...
 public:
  ...
  // Minimum render-task stack bytes that have never been used (ESP-IDF returns
  // bytes). Feeds the periodic MEM log so the 16KB reservation can be
  // validated against a measured worst case. Returns -1 before begin().
  int renderTaskStackHighWaterBytes() const {
    return renderTaskHandle ? static_cast<int>(uxTaskGetStackHighWaterMark(renderTaskHandle)) : -1;
  }
  ...
  void goToBoot(bool forcePanelScrub = false);
