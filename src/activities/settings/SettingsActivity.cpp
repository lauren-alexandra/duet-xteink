...
#include "activities/apps/LauncherCustomizeActivity.h"
...
constexpr size_t controlsPowerMinCount = 4;
constexpr size_t controlsPowerMaxCount = 5;
...
void drawSystemVersionFooter(const GfxRenderer& renderer, const int pageWidth, const int pageHeight,
  ...
  const std::string label = DUET_PRODUCT_NAME " " DUET_VERSION;
...
std::string formatSettingValue(const SettingInfo& setting) {
  ...
  if (setting.valuePtr == &CrossPointSettings::readingGoalMinutes) {
    char valueBuffer[24];
    snprintf(valueBuffer, sizeof(valueBuffer), "%u min", static_cast<unsigned>(SETTINGS.*(setting.valuePtr)));
    return valueBuffer;
  }
...
void SettingsActivity::rebuildSettingsLists() {
  ...
  displayHomeStatsSettings.clear();
  ...
  displayHomeStatsSettings = buildDisplayHomeStatsSettingsList(allSettings);
    ...
    case 0:
      switch (activeSubmenu) {
        case SettingAction::DisplaySleepScreen:
          ...
          break;
        case SettingAction::DisplayHomeStats:
          currentSettings = &displayHomeStatsSettings;
          break;
        default:
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    switch (setting.action) {
      ...
      case SettingAction::CustomiseStatusBar:
        ...
        break;
      case SettingAction::CustomizeLauncher:
        startActivityForResult(std::make_unique<LauncherCustomizeActivity>(renderer, mappedInput), resultHandler);
        break;
      ...
      case SettingAction::RestoreStats:
        startActivityForResult(std::make_unique<RestoreStatsActivity>(renderer, mappedInput), resultHandler);
        break;
      case SettingAction::StatsDate:
        startActivityForResult(std::make_unique<StatsDateActivity>(renderer, mappedInput), resultHandler);
        break;
      case SettingAction::ResetGlobalStats:
        startActivityForResult(
            ...
            [this](const ActivityResult& result) {
              if (!result.isCancelled) {
                const bool globalReset = GlobalReadingStats::resetLocal();
                const bool journalReset = ReadingJournal::resetLocal();
                if (!globalReset || !journalReset) {
      ...
      case SettingAction::ClearCache:
        ...
        break;
      case SettingAction::PurgeOrphanCaches:
        startActivityForResult(std::make_unique<PurgeOrphanCachesActivity>(renderer, mappedInput), resultHandler);
        break;
...
void SettingsActivity::render(RenderLock&&) {
  ...
  // Draw the Duet version label at the bottom of the System tab.
