...
void SdFirmwareUpdateActivity::performUpdate() {
  ...
  requestUpdateAndWait();
  delay(1500);
  // The newly installed firmware recognizes its X3/X4 + version marker on the
  // next boot and performs the post-update panel cleanup there. Keeping the
  // scrub on that side avoids an extra full waveform before every restart.
  ESP.restart();
}
...
void SdFirmwareUpdateActivity::render(RenderLock&&) {
  ...
  if (state == State::VALIDATING) {
    ...
    // The hint is wider than the screen; wrap it instead of clipping both edges.
