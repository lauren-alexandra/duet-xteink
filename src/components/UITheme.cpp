...
void UITheme::setTheme(CrossPointSettings::UI_THEME type) {
  switch (type) {
    ...
    case CrossPointSettings::UI_THEME::DASHBOARD:
      ...
      break;
    case CrossPointSettings::UI_THEME::READING_HOME:
      LOG_DBG("UI", "Using Reading Home theme");
      currentTheme = std::make_unique<ReadingHomeTheme>();
      currentMetrics = &ReadingHomeMetrics::values;
      break;
    default:
...
std::string UITheme::getCoverThumbPath(const std::string& coverBmpPath, int width, int height) {
  ...
  if (pos != std::string::npos) {
    if (hasWidthPlaceholder) {
      ...
      if (!Storage.existsForRead(thumbPath) && Storage.existsForRead(legacyPath)) {
        return legacyPath;
      }
    }
  }

  // Stored recent/favorite covers may still carry the retired in-cache-dir
  // template (DUET_BOOKS_ROOT_PATH "/<cache>/thumb_WxH.bmp"). Thumbnails now live in
  // small Duet thumbnail shards. This render helper performs read-path
  // translation only: migration writes belong to the input/storage task.
  // The legacy existence probe scans the crowded cache directory, so it runs
  // AT MOST ONCE per exact path per session: cover browsers re-request the
  // same paths every loop pass, and re-probing froze large shelves.
  const size_t thumbToken = thumbPath.rfind("/thumb_");
  if (thumbToken != std::string::npos && thumbPath.rfind(DUET_BOOKS_ROOT_PATH "/", 0) == 0) {
    const std::string cacheDir = thumbPath.substr(0, thumbToken);
    const size_t slash = cacheDir.find_last_of('/');
    const std::string dirName = slash == std::string::npos ? cacheDir : cacheDir.substr(slash + 1);
    const size_t underscore = dirName.find_last_of('_');
    const std::string hash = underscore == std::string::npos ? dirName : dirName.substr(underscore + 1);
    if (!hash.empty()) {
      const std::string tail = thumbPath.substr(thumbToken + 7);  // after "/thumb_"
      const std::string shardDir = ThumbnailCache::shardDirForHash(hash);
      const std::string shardedPath = shardDir + "/" + hash + "_" + tail;
      if (Storage.existsForRead(shardedPath)) {
        return shardedPath;
      }
      const std::string oldShardPath =
          ThumbnailCache::legacyShardDirForHash(hash) + "/" + hash + "_" + tail;
      if (oldShardPath != shardedPath && Storage.existsForRead(oldShardPath)) {
        return shardedPath;
      }
      // Hash keys, not path strings: 512 stored paths cost ~33 KB of
      // permanent heap as std::strings — enough to starve the home screen's
      // cover buffer — versus 4 KB as FNV-1a hashes.
      static std::mutex probeMutex;
      static std::vector<uint64_t> probedLegacy;
      uint64_t pathKey = 1469598103934665603ULL;
      for (const char c : thumbPath) {
        pathKey ^= static_cast<uint8_t>(c);
        pathKey *= 1099511628211ULL;
      }
      {
        std::lock_guard<std::mutex> lock(probeMutex);
        for (const uint64_t seen : probedLegacy) {
          if (seen == pathKey) {
            return shardedPath;  // already probed this session; treat as pending generation
          }
        }
        if (probedLegacy.size() < 512) probedLegacy.push_back(pathKey);
      }
      if (!Storage.existsForRead(thumbPath)) {
        return shardedPath;  // neither exists: report the new home so generation lands there
      }
      return thumbPath;
    }
  }
