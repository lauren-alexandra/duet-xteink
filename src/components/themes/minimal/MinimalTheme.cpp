...
// One picker-driven row: icon style for the streak / reader-type kinds,
// "value · label" text for anything else from the Home Stats catalog.
void drawPickedStatsRow(const GfxRenderer& renderer, const GlobalReadingStats& globalStats,
                        const HomeStatContext& statCtx, const uint8_t kind, const int regionTop,
                        const int regionBottom) {
  char value[48];
  char label[48];
  // Person-level rows read the merged history once peers exist.
  const GlobalReadingStats& personStats = statCtx.allStats != nullptr ? *statCtx.allStats : globalStats;
  if (kind == CrossPointSettings::HOME_STAT_READER_TYPE) {
    drawCenteredStatsRow(renderer, readerTypeIcon(personStats), kStatsFooterReaderIconSize,
                         readerTypeLabel(personStats), regionTop, regionBottom);
    return;
  }
  if (kind == CrossPointSettings::HOME_STAT_STREAK) {
    formatStreakStat(personStats, value, sizeof(value));
    drawCenteredStatsRow(renderer, StreakIcon, kStatsFooterStreakIconSize, value, regionTop, regionBottom);
    return;
  }
  int valueFontId = UI_10_FONT_ID;
  if (!formatHomeStat(kind, statCtx, value, sizeof(value), label, sizeof(label), valueFontId)) return;
  char row[96];
  snprintf(row, sizeof(row), "%s  %s", value, label);
  drawCenteredStatsRow(renderer, nullptr, 0, row, regionTop, regionBottom);
}
...
void drawStatsOverlay(const GfxRenderer& renderer, const GlobalReadingStats& globalStats, const Rect& coverRect,
  ...
  HomeStatContext statCtx = buildHomeStatContext(emptyStats, progressPercent, &globalStats);
  GlobalReadingStats aggregated{};
  if (homeStatSelectionWantsAllDevices()) {
    aggregated = GlobalReadingStats::loadAggregated(globalStats);
    statCtx.allStats = &aggregated;
  }

  const int readerRegionTop = 0;
  const int readerRegionBottom = coverImageRectForFrame(coverRect).y;
  drawPickedStatsRow(renderer, globalStats, statCtx, SETTINGS.homeFooterRight, readerRegionTop, readerRegionBottom);
  ...
  drawPickedStatsRow(renderer, globalStats, statCtx, SETTINGS.homeFooterLeft, streakRegionTop, streakRegionBottom);
}
...
std::string coverPathForImageRect(const RecentBook& book, const Rect& imageRect) {
  ...
  if (FsHelpers::hasEpubExtension(book.path)) {
    return Epub(book.path, DUET_BOOKS_ROOT_PATH "").getAdaptiveThumbBmpPath(imageRect.width, imageRect.height);
  }
...
void MinimalTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
                                       ...
                                       uint32_t currentBookWordCount) const {
  (void)selectorIndex;
  (void)bufferRestored;
  (void)globalStats;
  (void)currentChapterTitle;
  (void)currentBookWordCount;
...
void MinimalTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  const int contentTop = std::max(0, rect.y);
  const int contentBottom = std::min(renderer.getScreenHeight() - MinimalMetrics::values.buttonHintsHeight, rect.y + rect.height);
  const int availableHeight = std::max(0, contentBottom - contentTop);
  const int panelY = contentTop + std::max(0, (availableHeight - panelH) / 2);
