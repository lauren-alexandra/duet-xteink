...
constexpr char BOOKMARKS_DIR[] = DUET_STATE_ROOT_PATH "/bookmarks";
...
bool BookmarkStore::loadForBook(const std::string& filePath, const std::string& title, const std::string& author,
  ...
  if (!hasCurrentFile && !hasLegacyFile) {
    if (bookType == "epub" && isInReadFolder(filePath) && Storage.existsForRead(BOOKMARKS_DIR)) {
