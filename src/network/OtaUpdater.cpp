...
namespace {
#ifndef DUET_OTA_RELEASE_URL
#ifdef CROSSINK_OTA_RELEASE_URL
#define DUET_OTA_RELEASE_URL CROSSINK_OTA_RELEASE_URL
#else
#define DUET_OTA_RELEASE_URL "https://api.github.com/repos/lauren-alexandra/duet-xteink/releases?per_page=10"
#endif
#endif

constexpr char latestReleaseUrl[] = DUET_OTA_RELEASE_URL;

constexpr char firmwareAssetStem[] = "Duet-" DUET_ARTIFACT_DEVICE;
constexpr char firmwareAssetName[] = "Duet-" DUET_ARTIFACT_DEVICE ".bin";
#ifdef CROSSPOINT_FIRMWARE_VARIANT
constexpr char legacyFirmwareAssetStem[] = "firmware-" CROSSPOINT_FIRMWARE_VARIANT;
constexpr char legacyFirmwareAssetName[] = "firmware-" CROSSPOINT_FIRMWARE_VARIANT ".bin";
#endif
...
bool isMatchingFirmwareAssetName(const char* assetName) {
  ...
  if (startsWith(assetName, firmwareAssetStem) && assetName[strlen(firmwareAssetStem)] == '-' &&
      endsWith(assetName, binSuffix)) {
    return true;
  }
#ifdef CROSSPOINT_FIRMWARE_VARIANT
  if (strcmp(assetName, legacyFirmwareAssetName) == 0) return true;
  if (startsWith(assetName, legacyFirmwareAssetStem) && assetName[strlen(legacyFirmwareAssetStem)] == '-' &&
      endsWith(assetName, binSuffix)) {
    return true;
  }
#endif
...
esp_err_t http_client_set_header_cb(esp_http_client_handle_t http_client) {
  return esp_http_client_set_header(http_client, "User-Agent", "Duet-ESP32-" DUET_VERSION);
}
...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  ...
  LOG_DBG("OTA", "Checking for update (current: %s)", DUET_VERSION);
  ...
  esp_err = esp_http_client_set_header(client_handle, "User-Agent", "Duet-ESP32-" DUET_VERSION);
  if (esp_err != ESP_OK) {
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == DUET_VERSION) {
    return false;
  }

  const int comparison = DuetVersion::compare(latestVersion.c_str(), DUET_VERSION);
  LOG_DBG("OTA", "Version comparison latest=%s current=%s result=%d", latestVersion.c_str(), DUET_VERSION,
          comparison);
  return comparison > 0;
}
