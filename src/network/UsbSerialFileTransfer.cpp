...
constexpr const char* TEMP_UPLOAD_PATH = DUET_STATE_ROOT_PATH "/usb-upload.tmp";
constexpr const char* INTERNAL_DIR = DUET_BOOKS_ROOT_PATH "";
...
bool readExact(uint8_t* buffer, size_t length, uint32_t timeoutMs, size_t* receivedOut = nullptr,
  ...
  while (received < length) {
    ...
    if (available > 0) {
      ...
      // The native simulator's HWCDC shim exposes only Stream::read(), unlike
      // the ESP implementation's bulk-read overload.
      size_t bytesRead = 0;
