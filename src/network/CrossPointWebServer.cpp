...
void CrossPointWebServer::handleFontList() const {
  ...
  for (const auto& family : families) {
    ...
    for (const auto& file : family.files) {
      ...
      const std::string fileName = family.fileNameForFile(file);
      const std::string filePath = family.pathForFile(file);
      fileObj["name"] = fileName;

      // Stat the file for size
      HalFile f;
      if (Storage.openFileForRead("WEB", filePath.c_str(), f)) {
...
void CrossPointWebServer::handleFontUploadData() {
  ...
  switch (upload.status) {
    case UPLOAD_FILE_START: {
      ...
      // this an attacker could supply "../../.duet/state/settings.json" as
