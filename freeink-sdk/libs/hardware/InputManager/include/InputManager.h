...
class InputManager {
 public:
  ...
  void update();
  // Poll during a long cooperative operation without losing the edge before
  // the next normal main-loop update. Preserved edges are replayed once.
  void updatePreservingEvents();
 ...
 private:
  ...
  void updateImpl(bool preserveEventsForNextUpdate);
  ...
  uint8_t preservedPressedEvents;
  uint8_t preservedReleasedEvents;
  ...
  bool preservedTouchPressedEvent = false;
  bool preservedTouchReleasedEvent = false;
  bool preservedTouchHomeKeyEvent = false;
