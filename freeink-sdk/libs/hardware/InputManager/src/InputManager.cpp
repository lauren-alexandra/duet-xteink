...
InputManager::InputManager()
    : currentState(0),
      ...
      pressedEvents(0),
      releasedEvents(0),
      preservedPressedEvents(0),
      preservedReleasedEvents(0),
      lastDebounceTime(0),
      buttonPressStart(0),
      buttonPressFinish(0),
...
void InputManager::update() { updateImpl(false); }

void InputManager::updatePreservingEvents() { updateImpl(true); }

void InputManager::updateImpl(const bool preserveEventsForNextUpdate) {
  const unsigned long currentTime = millis();
  const uint8_t previousPressedEvents = pressedEvents;
  const uint8_t previousReleasedEvents = releasedEvents;
  const bool previousTouchPressedEvent = touchPressedEvent;
  const bool previousTouchReleasedEvent = touchReleasedEvent;
  const bool previousTouchHomeKeyEvent = touchHomeKeyEvent;
  ...
  if (preserveEventsForNextUpdate) {
    pressedEvents |= previousPressedEvents;
    releasedEvents |= previousReleasedEvents;
    touchPressedEvent = touchPressedEvent || previousTouchPressedEvent;
    touchReleasedEvent = touchReleasedEvent || previousTouchReleasedEvent;
    touchHomeKeyEvent = touchHomeKeyEvent || previousTouchHomeKeyEvent;
    preservedPressedEvents |= pressedEvents;
    preservedReleasedEvents |= releasedEvents;
    preservedTouchPressedEvent = preservedTouchPressedEvent || touchPressedEvent;
    preservedTouchReleasedEvent = preservedTouchReleasedEvent || touchReleasedEvent;
    preservedTouchHomeKeyEvent = preservedTouchHomeKeyEvent || touchHomeKeyEvent;
    return;
  }

  pressedEvents |= preservedPressedEvents;
  releasedEvents |= preservedReleasedEvents;
  touchPressedEvent = touchPressedEvent || preservedTouchPressedEvent;
  touchReleasedEvent = touchReleasedEvent || preservedTouchReleasedEvent;
  touchHomeKeyEvent = touchHomeKeyEvent || preservedTouchHomeKeyEvent;
  preservedPressedEvents = 0;
  preservedReleasedEvents = 0;
  preservedTouchPressedEvent = false;
  preservedTouchReleasedEvent = false;
  preservedTouchHomeKeyEvent = false;
}
