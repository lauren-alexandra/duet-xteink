...
Duet is firmware for the Xteink X4 and X3, built with PlatformIO targeting the ESP32-C3 microcontroller.
...
Current SD-card persistence lives under the canonical `/.duet/` namespace: hot state, durable per-book records, disposable caches, reading-stat backups, and migration records have separate subdirectories. Legacy `/.crossink` and `/.crosspoint` paths remain compatibility inputs. See the file inventory in `docs/data-cache.md`; for binary formats, see `docs/file-formats.md`.
