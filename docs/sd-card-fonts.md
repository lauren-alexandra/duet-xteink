...
Alpha.7 does not currently include a separate expanded Duet font ZIP.

- **On the reader:** connect to Wi-Fi and open **Settings > Reader > Font Options > Manage Fonts**. This downloads from the credited 24-family CrossInk compatibility catalog.
- **On a computer:** download compatible ready-made `.cpfont` files from the [CrossInk font releases](https://github.com/uxjulia/crossink-fonts/releases), then copy them to `/.fonts/` or `/fonts/`.
- **Build additional open-source families:** use Duet's checked-in conversion tools and the source projects recorded under [Font Sources](font-sources.md).

The larger generated collection used during Duet development is not attached to the Alpha.7 release. There is therefore no current public link that downloads every font shown in Duet's development screenshots.
...
1. Obtain compatible `.cpfont` files from the on-device catalog, the [CrossInk font releases](https://github.com/uxjulia/crossink-fonts/releases), or your own conversion.
