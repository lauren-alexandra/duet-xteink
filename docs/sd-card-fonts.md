...
1. Obtain compatible `.cpfont` files. During Alpha.7, the public ready-made set is the credited 24-family [CrossInk font release](https://github.com/uxjulia/crossink-fonts/releases). The expanded collection used on Duet's development cards is not yet a public release asset because its generated files and per-family notices are still being reviewed.
...
The Alpha.7 in-device downloader reads the standard 24-family catalog generated by Duet's checked-in `lib/EpdFont/scripts/sd-fonts.yaml`, using CrossInk's existing compatibility endpoint. That is an upstream compatibility service, not Duet's full font collection.

Duet's larger development-card collection is not being presented as a public download until every generated family has a complete source record, redistribution notice, and license in the same archive. This keeps the firmware release moving without publishing a font bundle whose licensing paperwork is unfinished. The source and publication gate are recorded in [Font Sources and Redistribution](../FONT_SOURCES.md).
