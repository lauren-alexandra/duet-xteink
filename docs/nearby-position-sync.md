...
Duet can share the current EPUB reading position between two nearby X3/X4 readers running Duet. The sync is direct reader-to-reader over ESP-NOW; it does not use WiFi, a server, an account, or KOReader's online sync service.
...
- Both readers must be running Duet.
...
Before starting the sync, Duet saves the current reader progress. The sender then broadcasts the current EPUB position over ESP-NOW. The receiver checks that the incoming packet is from a compatible Duet nearby-position protocol and that the book identity matches the EPUB currently open on the receiver.

If the book matches, the receiver maps the shared KOReader-compatible position back to Duet's local section and page model. When paragraph, list-item, or anchor details are available, Duet uses them to refine the target page.

After you press **Select**, Duet writes the received position to the normal EPUB progress file and returns to the reader at that position.
...
The other reader is using an incompatible nearby-position sync protocol. Update both readers to compatible Duet builds.
