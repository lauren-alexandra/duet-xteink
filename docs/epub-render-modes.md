...
Duet normally tries to render EPUBs with the full Duet renderer. Some EPUBs contain complex publisher styling, tables, or image rules that can use too much memory for the reader. Render modes let you keep reading those books by asking Duet to simplify the layout work for that book.
...
| --- | --- | --- | --- |
| Duet Default | Most books | Full Duet styling, detailed CSS, table layout, image sizing, publisher spacing, Bionic Reading, Guide Dots | Nothing by default |
| Balanced | Books where Duet has fallen back automatically, or books you want to start in a lighter mode | Publisher spacing, image sizing, decorations, Bionic Reading, Guide Dots | Complex CSS lookups and table layout |
...
Most users can leave books on **Duet Default**. If a book runs out of memory while building a section, Duet will automatically try **Balanced** and then **Light**. Manual mode changes are mainly for choosing a lighter mode before opening a known-problem book, or for changing the saved mode after Duet has already fallen back.

## Duet Default

Duet Default is the normal renderer.

It tries to preserve the book's layout and publisher styling as much as Duet supports, including:
...
Use Balanced when Duet has already fallen back to it, or when you want a known-problem book to start with lighter rendering immediately. The page may not match the publisher layout exactly, but it should still keep more of the book's visual styling than Light.
...
Use Light when Duet has already fallen back to it, or when you want a known-problem book to start in the safest mode immediately. Light may look less like the publisher's original layout, but it gives Duet the best chance of rendering the book on limited memory.

## Safe Mode

Safe Mode is an automatic final fallback, not a selectable render mode.

If Duet cannot build a section in Light because memory is still too tight, it tries Safe Mode once. Safe Mode uses Light rendering and also turns off:

- Embedded styles
- Bionic Reading
- Guide Dots

When Safe Mode succeeds, Duet saves those settings for that book. The reader briefly shows **Safe Mode** over the current page. Your other books keep their own reader settings.

## Automatic Fallback

If a book starts in Duet Default and a section runs out of memory while building, Duet automatically retries in this order:
...
If the book starts in Balanced, Duet can fall back to Light and then Safe Mode. If the book starts in Light, Duet can still try Safe Mode.

When a fallback succeeds, Duet saves that successful mode for the book. The next time you open the same book, it starts in the saved mode.
...
You can change a book's render mode before opening it. This is optional because Duet already falls back automatically, but it is useful when you know a book is difficult and want it to start in a specific mode:
