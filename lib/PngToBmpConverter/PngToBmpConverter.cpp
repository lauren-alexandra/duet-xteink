...
bool PngToBmpConverter::pngFileToBmpStreamInternal(FsFile& pngFile, Print& bmpOut, int targetWidth, int targetHeight,
  ...
  if (shouldCancel && shouldCancel(cancelContext)) {
    LOG_DBG("PNG", "PNG conversion cancelled before decode");
    return false;
  }
  ...
  // Heap-allocated: the context is ~4.2KB (readBuf + palette + inflate trees),
  // too large for the 8KB loop-task stack this runs on during cover generation.
  ...
  Burkes1BitDitherer* burkes1BitDitherer = nullptr;

  // Ditherer OOM degrades to the ordered-quantize fallback already used when
  // the pointers are null, instead of aborting the whole conversion.
  if (oneBit) {
    burkes1BitDitherer = new (std::nothrow) Burkes1BitDitherer(outWidth);
    if (burkes1BitDitherer && !burkes1BitDitherer->valid()) {
      delete burkes1BitDitherer;
      burkes1BitDitherer = nullptr;
    }
    if (!burkes1BitDitherer) LOG_ERR("PNG", "Ditherer OOM (width %d); using ordered quantize", outWidth);
    ...
    if (USE_ATKINSON) {
      ...
      if (!atkinsonDitherer) LOG_ERR("PNG", "Ditherer OOM (width %d); using ordered quantize", outWidth);
      ...
      if (!fsDitherer) LOG_ERR("PNG", "Ditherer OOM (width %d); using ordered quantize", outWidth);
    }
  }
  ...
  if (needsScaling) {
    ...
    if (!rowAccum || !rowCount) {
      LOG_ERR("PNG", "Failed to allocate scaling accumulators (width %d)", outWidth);
      delete[] rowAccum;
      delete[] rowCount;
      delete atkinsonDitherer;
      delete fsDitherer;
  ...
  for (uint32_t y = 0; y < height; y++) {
    if (shouldCancel && shouldCancel(cancelContext)) {
      LOG_DBG("PNG", "PNG conversion cancelled at row %u", y);
      success = false;
      break;
    }
    // Decode one scanline
    ...
    if (!needsScaling) {
      ...
      if (USE_8BIT_OUTPUT && !oneBit) {
        ...
        const bool reverse = burkes1BitDitherer && burkes1BitDitherer->isReverseRow();
        const int start = reverse ? outWidth - 1 : 0;
        const int end = reverse ? -1 : outWidth;
        const int step = reverse ? -1 : 1;
        for (int x = start; x != end; x += step) {
          const uint8_t bit =
              burkes1BitDitherer ? burkes1BitDitherer->processPixel(grayRow[x], x) : quantize1bit(grayRow[x], x, y);
          const int byteIndex = x / 8;
          const int bitOffset = 7 - (x % 8);
          rowBuffer[byteIndex] |= (bit << bitOffset);
        }
        if (burkes1BitDitherer) burkes1BitDitherer->nextRow();
      } else {
        for (int x = 0; x < outWidth; x++) {
      ...
      while (srcY_fp >= nextOutY_srcStart && currentOutY < outHeight) {
        ...
        if (USE_8BIT_OUTPUT && !oneBit) {
          ...
          for (int x = start; x != end; x += step) {
            const uint8_t gray = (rowCount[x] > 0) ? (rowAccum[x] / rowCount[x]) : 0;
            const uint8_t bit =
                burkes1BitDitherer ? burkes1BitDitherer->processPixel(gray, x) : quantize1bit(gray, x, currentOutY);
            const int byteIndex = x / 8;
            const int bitOffset = 7 - (x % 8);
            rowBuffer[byteIndex] |= (bit << bitOffset);
          }
