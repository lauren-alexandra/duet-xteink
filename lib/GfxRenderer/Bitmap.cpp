...
BmpReaderError Bitmap::parseHeaders() {
  ...
  if (highColor && dithering) {
    // Ditherer OOM degrades to plain quantization (null pointers take the
    // non-dithered branch in readNextRow) instead of aborting.
    if (USE_ATKINSON) {
      ...
      if (!atkinsonDitherer) LOG_ERR("BMP", "Ditherer OOM (width %d); using plain quantize", width);
    } else {
      ...
      if (!fsDitherer) LOG_ERR("BMP", "Ditherer OOM (width %d); using plain quantize", width);
    }
  }
...
BmpReaderError Bitmap::readRawRows(uint8_t* data, const size_t dataSize) const {
  const size_t required = static_cast<size_t>(rowBytes) * static_cast<size_t>(height);
  if (data == nullptr || dataSize < required) return BmpReaderError::BufferTooSmall;
  const BmpReaderError rewindResult = rewindToData();
  if (rewindResult != BmpReaderError::Ok) return rewindResult;
  return file.read(data, required) == static_cast<int>(required) ? BmpReaderError::Ok
