...
class GfxRenderer {
 ...
 private:
  ...
  bool fadingFix;
  mutable uint16_t fastRefreshesSinceClear_ = 0;
 ...
 public:
  ...
  // Fast (partial-waveform) refreshes since the last FULL/HALF clear. Lets
  // callers decide whether a deghost scrub pass has residue worth clearing.
  uint16_t fastRefreshesSinceClear() const { return fastRefreshesSinceClear_; }

  void displayBuffer(HalDisplay::RefreshMode refreshMode = HalDisplay::FAST_REFRESH, bool turnOffScreen = false,
                     bool honorFadingFix = true) const;
  // Display a logical-orientation rectangle without repainting the whole panel.
  // The backend widens it to byte boundaries and safely falls back if needed.
  ...
  void drawPerspectiveBitmap1Bit(const uint8_t* rows, int sourceWidth, int sourceHeight, int sourceRowBytes,
                                 bool sourceTopDown, uint8_t blackPaletteIndex, int x, int y, int w, int hL,
                                 int hR) const;
  ...
  bool hasDistinctFontStyle(int fontId, EpdFontFamily::Style baseStyle, EpdFontFamily::Style emphasizedStyle) const;
  bool isSyntheticFontStyle(int fontId, EpdFontFamily::Style style) const;
  ...
  void displayCleanGrayscaleBase(bool turnOffScreen = false) const;
