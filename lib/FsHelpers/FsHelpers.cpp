...
void sortFileListByBookTitle(std::vector<std::string>& strs) {
  std::sort(begin(strs), end(strs), [](const std::string& str1, const std::string& str2) {
    const bool isDir1 = str1.back() == '/';
    const bool isDir2 = str2.back() == '/';
    if (isDir1 != isDir2) return isDir1;
    if (isDir1) return naturalLess(str1, str2);

    const int titleCmp = naturalCompare(bookTitleSortName(str1.c_str()), bookTitleSortName(str2.c_str()));
