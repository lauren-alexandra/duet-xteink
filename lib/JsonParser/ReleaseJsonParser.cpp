...
void ReleaseJsonParser::commitAsset() {
  ...
  if (!firstReleaseComplete && !firmwareFound && matchesFirmware) {
...
void ReleaseJsonParser::sOnKey(void* ctx, const char* key, size_t len) {
  ...
  switch (self->position) {
    case Position::TOP_LEVEL:
      if (!self->firstReleaseComplete && self->depth == (self->topLevelArray ? 2 : 1)) {
...
void ReleaseJsonParser::sOnString(void* ctx, const char* value, size_t len) {
  ...
  switch (self->lastKey) {
    case LastKey::TAG_NAME:
      if (!self->tagFound && !self->firstReleaseComplete && self->position == Position::TOP_LEVEL &&
          self->depth == (self->topLevelArray ? 2 : 1)) {
...
void ReleaseJsonParser::sOnObjectEnd(void* ctx) {
  ...
  switch (self->position) {
    case Position::TOP_LEVEL:
      if (self->topLevelArray && self->depth == 2) self->firstReleaseComplete = true;
...
void ReleaseJsonParser::sOnArrayStart(void* ctx) {
  ...
  switch (self->position) {
    case Position::TOP_LEVEL:
      if (!self->firstReleaseComplete && self->lastKey == LastKey::ASSETS &&
          self->depth == (self->topLevelArray ? 2 : 1)) {
        self->position = Position::IN_ASSETS_ARRAY;
      } else {
        if (self->depth == 0) self->topLevelArray = true;
        self->depth++;
      }
      self->lastKey = LastKey::NONE;
      break;
