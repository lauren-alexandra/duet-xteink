...
namespace {
constexpr const char* INDEX_ROOT = DUET_BOOKS_ROOT_PATH "";
constexpr const char* INDEX_DIR = DUET_FILE_INDEX_ROOT_PATH "";
...
bool FileIndex::open(const char* dirPath, AcceptFn accept, const SortMode requestedSortMode) {
  close();
  ...
  // The file browser is the sole writer while this activity is alive. Going
  // back to a folder we just left can therefore trust its previously verified
  // directory signature and avoid a second full FAT walk before opening the
  // on-card index.
  if (TrustedIndex* trusted = findTrustedIndex(dirPath, accept)) {
    if (loadExisting(dirPath, trusted->signature, trusted->dirs, trusted->files)) {
      trusted->useStamp = ++trustedIndexUseStamp;
      LOG_DBG("FIDX", "trusted index reused for %s (%u dirs, %u files)", dirPath, hdr.dirCount, hdr.fileCount);
      return true;
    }
    forgetTrustedIndex(dirPath);
  }
  ...
  const uint8_t sortByte = static_cast<uint8_t>(sortMode);
  signature = fnv1a32(&sortByte, sizeof(sortByte), signature);

  if (loadExisting(dirPath, signature, dirs, files)) {
    rememberTrustedIndex(dirPath, accept, signature, dirs, files);
  ...
  const bool built = build(dirPath, accept, signature, dirs, files);
  if (built) rememberTrustedIndex(dirPath, accept, signature, dirs, files);
  return built;
}

bool FileIndex::hasTrustedIndex(const char* dirPath, AcceptFn accept, const SortMode requestedSortMode) const {
  if (!dirPath) return false;
  for (const auto& trusted : trustedIndexes) {
    if (trusted.path[0] != '\0' && trusted.accept == accept && trusted.sortMode == requestedSortMode &&
        strcmp(trusted.path, dirPath) == 0) {
      return true;
    }
  }
  return false;
...
void FileIndex::forgetTrustedIndex(const char* dirPath) {
  if (!dirPath) return;
  for (auto& trusted : trustedIndexes) {
    if (trusted.path[0] != '\0' && strcmp(trusted.path, dirPath) == 0) {
      trusted = TrustedIndex{};
    }
  }
}

FileIndex::TrustedIndex* FileIndex::findTrustedIndex(const char* dirPath, AcceptFn accept) {
  if (!dirPath) return nullptr;
  for (auto& trusted : trustedIndexes) {
    if (trusted.path[0] != '\0' && trusted.accept == accept && trusted.sortMode == sortMode &&
        strcmp(trusted.path, dirPath) == 0) {
      return &trusted;
    }
  }
...
const FileIndex::TrustedIndex* FileIndex::findTrustedIndex(const char* dirPath, AcceptFn accept) const {
  if (!dirPath) return nullptr;
  for (const auto& trusted : trustedIndexes) {
    if (trusted.path[0] != '\0' && trusted.accept == accept && trusted.sortMode == sortMode &&
        strcmp(trusted.path, dirPath) == 0) {
...
void FileIndex::rememberTrustedIndex(const char* dirPath, AcceptFn accept, uint32_t signature, uint32_t dirs,
                                     uint32_t files) {
  if (!dirPath || strlen(dirPath) >= sizeof(TrustedIndex::path)) return;

  TrustedIndex* target = findTrustedIndex(dirPath, accept);
  if (!target) {
    target = &trustedIndexes[0];
    for (auto& candidate : trustedIndexes) {
      if (candidate.path[0] == '\0') {
        target = &candidate;
        break;
      }
      if (candidate.useStamp < target->useStamp) target = &candidate;
    }
  }

  strncpy(target->path, dirPath, sizeof(target->path) - 1);
  ...
  target->useStamp = ++trustedIndexUseStamp;
}
...
bool FileIndex::loadExisting(const char* dirPath, uint32_t signature, uint32_t dirs, uint32_t files) {
  ...
  valid = valid && h.sortMode == static_cast<uint8_t>(sortMode);
...
void FileIndex::makeKey(RunRecord& rec, bool isDir, const char* name) const {
  ...
  FsHelpers::naturalSortKey((!isDir && sortMode == SortMode::BookTitle) ? FsHelpers::bookTitleSortName(name) : name,
                            rec.key + 1, sizeof(rec.key) - 1);
}
...
bool FileIndex::build(const char* dirPath, AcceptFn accept, uint32_t signature, uint32_t dirs, uint32_t files) {
  ...
  const uint8_t sortByte = static_cast<uint8_t>(sortMode);
  sig = fnv1a32(&sortByte, sizeof(sortByte), sig);
