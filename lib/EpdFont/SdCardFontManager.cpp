...
bool SdCardFontManager::loadFamily(const SdCardFontFamilyInfo& family, GfxRenderer& renderer, uint8_t targetPointSize,
  ...
  const std::string selectedPath = family.pathForFile(*selected);

  auto* font = new (std::nothrow) SdCardFont();
  if (!font) {
    LOG_ERR("SDMGR", "Failed to allocate SdCardFont for %s", selectedPath.c_str());
    return false;
  }

  if (!font->load(selectedPath.c_str())) {
    LOG_ERR("SDMGR", "Failed to load %s", selectedPath.c_str());
    delete font;
    return false;
  }
  ...
  if (renderer.getFontMap().count(fontId) != 0) {
    LOG_ERR("SDMGR", "Font ID %d collides with existing font, skipping %s", fontId, selectedPath.c_str());
    delete font;
    return false;
  }
  ...
  LOG_DBG("SDMGR", "Loaded %s size=%u id=%d styles=%u (target=%u step=%u)", selectedPath.c_str(), selected->pointSize,
