...
constexpr uint16_t SYNTHETIC_STYLE_MASK = 0x0F00;
constexpr uint8_t SYNTHETIC_STYLE_SHIFT = 8;
...
bool SdCardFont::load(const char* path) {
  ...
  const uint16_t fileFlags = readU16(headerBuf + 10);
  bool is2Bit = (fileFlags & 1) != 0;
  syntheticStyleMask_ = static_cast<uint8_t>((fileFlags & SYNTHETIC_STYLE_MASK) >> SYNTHETIC_STYLE_SHIFT);
...
bool SdCardFont::isSyntheticStyle(const uint8_t style) const {
  const uint8_t resolved = resolveStyle(style) & (MAX_STYLES - 1);
  return (syntheticStyleMask_ & static_cast<uint8_t>(1u << resolved)) != 0;
}
