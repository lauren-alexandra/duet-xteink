...
def rasterize_font_style(fontfile, size, intervals, style_id=0, force_autohint=False,
                         fallback_fontfile=None, synthetic_effects=frozenset()):
    ...
    # fallback_fontfile may be a single path or a list; faces are tried in
    # order, so put general text fallbacks before symbol/emoji ones.
    fallback_faces = []
    if fallback_fontfile:
        fallback_files = list(fallback_fontfile) if isinstance(fallback_fontfile, (list, tuple)) \
            else [fallback_fontfile]
        for fb_file in fallback_files:
            fb_face = freetype.Face(fb_file)
            fb_face.set_char_size(size << 6, size << 6, 150, 150)
            fallback_faces.append(fb_face)

    synthetic_effects = frozenset(synthetic_effects)
    synthetic_bold = "bold" in synthetic_effects
    synthetic_italic = "italic" in synthetic_effects

    # FreeType's slot synthesis helpers operate on an outline. True font
    # faces can keep the existing one-step render path; synthetic faces load
    # the outline, transform it, and render only after the transformation.
    load_flags = freetype.FT_LOAD_DEFAULT if synthetic_effects else freetype.FT_LOAD_RENDER
    if force_autohint:
        load_flags |= freetype.FT_LOAD_FORCE_AUTOHINT

    def render_loaded_glyph(loaded_face):
        if not synthetic_effects:
            return loaded_face

        slot = loaded_face.glyph._FT_GlyphSlot
        if synthetic_italic:
            freetype.FT_GlyphSlot_Oblique(slot)
        if synthetic_bold:
            freetype.FT_GlyphSlot_Embolden(slot)
        loaded_face.glyph.render(freetype.FT_RENDER_MODE_NORMAL)
        return loaded_face
    ...
    def load_glyph(code_point):
        ...
        if glyph_index > 0:
            ...
            return render_loaded_glyph(face)
        for fb_face in fallback_faces:
            ...
            if fallback_glyph_index > 0:
                ...
                return render_loaded_glyph(fb_face)
        return None
    ...
    for i_start, i_end in intervals:
        for code_point in range(i_start, i_end + 1):
            ...
            if synthetic_bold:
                # Emboldening expands the rendered slot. Preserve the font's
                # linear advance unless FreeType reports that the heavier
                # bitmap needs more room, preventing adjacent glyph clipping.
                advance_x = max(advance_x, fp4_from_ft26_6(f.glyph.advance.x))
...
def generate_cpfont_multistyle(style_fonts, size, intervals, output_path,
                               ...
                               synthetic_style_effects=None):
    """Generate a multi-style v4 .cpfont file.

    style_fonts: dict of {style_id: fontfile_path} e.g. {0: "Regular.ttf", 2: "Italic.ttf"}
    fallback_style_fonts: optional dict of {style_id: fallback path or list of paths tried in order}
    """
    MAGIC = b"CPFONT\x00\x00"
    HEADER_SIZE = 32
    STYLE_TOC_ENTRY_SIZE = 32
    flags = 1  # bit 0: always 2-bit greyscale
    ...
    synthetic_style_effects = synthetic_style_effects or {}
    # Bits 8-11 carry a backward-compatible mask of generated styles. Existing
    # v4 readers ignore these previously reserved flag bits; updated readers use
    # them to label simulated faces honestly in the picker.
    for style_id in synthetic_style_effects:
        flags |= 1 << (8 + style_id)
    for style_id in sorted(style_fonts.keys()):
        fontfile = style_fonts[style_id]
        fallback_fontfile = fallback_style_fonts.get(style_id)
        effects = synthetic_style_effects.get(style_id, frozenset())
        effect_note = f" (synthetic: {'+'.join(sorted(effects))})" if effects else ""
        print(f"  Rasterizing style {style_id}{effect_note}...", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--render-scale", dest="render_scale", type=float, default=1.0,
                        help="Rasterize glyphs at a multiple of the filename point size (default: 1.0).")
    ...
    parser.add_argument("--fallback-regular", dest="fallback_regular", action="append",
                        help="Fallback font file for regular style. May be repeated; tried in order.")
    parser.add_argument("--fallback-bold", dest="fallback_bold", action="append",
                        help="Fallback font file for bold style. May be repeated; tried in order.")
    parser.add_argument("--fallback-italic", dest="fallback_italic", action="append",
                        help="Fallback font file for italic style. May be repeated; tried in order.")
    parser.add_argument("--fallback-bolditalic", dest="fallback_bolditalic", action="append",
                        help="Fallback font file for bold-italic style. May be repeated; tried in order.")
    parser.add_argument(
        "--synthetic-style", action="append", default=[], metavar="STYLE=EFFECTS",
        help=("Mark an output face as synthetic, for example "
              "'italic=italic' or 'bolditalic=bold+italic'. May be repeated."),
    )

    args = parser.parse_args()

    if not 0.5 <= args.render_scale <= 2.0:
        parser.error("--render-scale must be between 0.5 and 2.0")
    ...
    style_name_to_id = {"regular": 0, "bold": 1, "italic": 2, "bolditalic": 3}
    synthetic_style_effects = {}
    for spec in args.synthetic_style:
        try:
            style_name, effect_spec = spec.split("=", 1)
        except ValueError:
            parser.error(f"invalid --synthetic-style '{spec}'; expected STYLE=EFFECTS")
        style_name = style_name.strip().lower()
        effects = frozenset(part.strip().lower() for part in effect_spec.split("+") if part.strip())
        if style_name not in style_name_to_id:
            parser.error(f"unknown synthetic style '{style_name}'")
        if not effects or not effects <= {"bold", "italic"}:
            parser.error(f"invalid synthetic effects in '{spec}'; use bold, italic, or bold+italic")
        style_id = style_name_to_id[style_name]
        if style_id not in style_fonts:
            parser.error(f"synthetic style '{style_name}' has no matching --{style_name} font source")
        synthetic_style_effects[style_id] = effects
    ...
    total_size = 0
    for sz in sizes:
        render_size = max(1, round(sz * args.render_scale))
        if args.output and len(sizes) == 1:
            output_path = args.output
        else:
            filename = f"{font_name}_{sz}.cpfont"
            output_path = os.path.join(output_dir, filename)
        scale_note = f", render size {render_size}" if render_size != sz else ""
        print(
            f"Generating {output_path} (size {sz}{scale_note}, {len(style_fonts)} style(s), v4)...",
            file=sys.stderr,
        )
        total_size += generate_cpfont_multistyle(
            style_fonts, render_size, intervals, output_path,
            force_autohint=args.force_autohint,
            fallback_style_fonts=fallback_style_fonts,
            synthetic_style_effects=synthetic_style_effects)
