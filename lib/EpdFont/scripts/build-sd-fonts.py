...
# Fallback chain, tried in order after the family's own faces: general text
# glyphs first, then text symbols (card suits ♥, arrows, geometry), then emoji.
# NotoSans alone left symbol gaps — U+2665 in shipped books rendered as the
# missing-glyph marker.
FALLBACK_FONTS = (
    DEFAULT_FALLBACK_FONT,
    EPDFONTS_DIR / "builtinFonts/source/NotoSymbols/NotoSansSymbols-Regular.ttf",
    EPDFONTS_DIR / "builtinFonts/source/NotoSymbols2/NotoSansSymbols2-Regular.ttf",
...
# as the built-in reader fonts, plus the common symbol blocks so the fallback
# chain can actually supply them. Families can still add extra script presets.
PATCHED_INTERVAL_PRESETS = ("builtin", "symbols")
...
def validate_config(families: list[dict]) -> list[str]:
    ...
    for family in families:
        ...
        try:
            render_scale = float(family.get("render_scale", 1.0))
            if not 0.5 <= render_scale <= 2.0:
                errors.append(
                    f"{family_name}: render_scale must be between 0.5 and 2.0"
                )
        except (TypeError, ValueError):
            errors.append(f"{family_name}: render_scale must be a number")
        synthesize_missing = family.get("synthesize_missing_styles", False)
        if not isinstance(synthesize_missing, bool):
            errors.append(f"{family_name}: synthesize_missing_styles must be true or false")
        if synthesize_missing and "regular" not in family.get("styles", {}):
            errors.append(
                f"{family_name}: synthesize_missing_styles requires a regular style"
            )
        for style_name, style_spec in family.get("styles", {}).items():
            if style_name not in STYLE_NAMES:
                errors.append(f"{family_name}: unknown style '{style_name}'")
...
def extract_static_instance(source_path: Path, axes: dict, family_name: str, style_name: str) -> Path:
    ...
    try:
        try:
            ...
            # Older Apple variable fonts such as Skia predate STAT. Their
            # outlines can still be instanced correctly; only the optional
            # automatic style-name rewrite requires that table.
            if "no STAT table" not in str(exc):
                raise
...
def build_family(
    ...
    # Fill only absent faces. Prefer a real related face as the source so a
    # missing bold-italic can add slant to real bold or weight to real italic,
    # rather than applying both effects to regular unnecessarily.
    synthetic_specs = []
    if family.get("synthesize_missing_styles", False):
        regular = resolved_styles["regular"]
        if "bold" not in resolved_styles:
            resolved_styles["bold"] = regular
            synthetic_specs.append("bold=bold")
        if "italic" not in resolved_styles:
            resolved_styles["italic"] = regular
            synthetic_specs.append("italic=italic")
        if "bolditalic" not in resolved_styles:
            if "bold" in styles:
                resolved_styles["bolditalic"] = resolved_styles["bold"]
                synthetic_specs.append("bolditalic=italic")
            elif "italic" in styles:
                resolved_styles["bolditalic"] = resolved_styles["italic"]
                synthetic_specs.append("bolditalic=bold")
            else:
                resolved_styles["bolditalic"] = regular
                synthetic_specs.append("bolditalic=bold+italic")
    ...
    if has_any_multi and len(resolved_styles) > 1:
        ...
        for style_name, font_path in resolved_styles.items():
            ...
            for fallback in FALLBACK_FONTS:
                cmd.extend([f"--fallback-{style_name}", str(fallback)])
    else:
        # Single-style mode
    ...
    for spec in synthetic_specs:
        cmd.extend(["--synthetic-style", spec])
    render_scale = float(family.get("render_scale", 1.0))
    if render_scale != 1.0:
        cmd.extend(["--render-scale", str(render_scale)])
...
def main():
    ...
    parser.add_argument(
        "--synthesize-missing-styles", action="store_true",
        help=("Generate synthetic bold/italic faces for every selected family "
              "whose config does not provide all four styles."),
    )
    parser.add_argument(
    ...
    if args.synthesize_missing_styles:
        for family in families:
            if set(family.get("styles", {})) != set(STYLE_NAMES):
                family["synthesize_missing_styles"] = True
