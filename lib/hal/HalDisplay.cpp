...
void HalDisplay::deghostClear() {
  HalSpiBus::Lock spiLock;
  einkDisplay.deghostClear();
}
...
void HalDisplay::displayCleanGrayscaleBase(bool turnOffScreen) {
  if (gpio.deviceIsX3()) {
    einkDisplay.requestResync(1);
  }
  einkDisplay.displayGrayscaleBase(EInkDisplay::FULL_REFRESH, turnOffScreen);
}
