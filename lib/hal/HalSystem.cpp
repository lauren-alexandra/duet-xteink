...
// Faulting PC and return address from the exception frame. CPU exceptions
// (bad load/store, stack-guard trips) never call panic_abort, so without
// these the report has no crash location at all.
RTC_NOINIT_ATTR uint32_t panicMepc;
RTC_NOINIT_ATTR uint32_t panicRa;
...
void IRAM_ATTR __wrap_panic_print_backtrace(const void* frame, int core) {
  ...
  panicMepc = (uint32_t)((RvExcFrame*)frame)->mepc;
  panicRa = (uint32_t)((RvExcFrame*)frame)->ra;
...
std::string getPanicInfo(bool full) {
  if (!full) {
    ...
    info += DUET_PRODUCT_NAME " version: " DUET_VERSION;
    info += "\n" DUET_PRODUCT_NAME " device: " DUET_ARTIFACT_DEVICE;
    info += "\n" DUET_PRODUCT_NAME " variant: " CROSSINK_FIRMWARE_VARIANT;
    info += "\n\nPanic reason: " + std::string(panicMessage);
    {
      char pcLine[48];
      snprintf(pcLine, sizeof(pcLine), "\nMEPC: 0x%08X  RA: 0x%08X", (unsigned)panicMepc, (unsigned)panicRa);
      info += pcLine;
    }
    info += "\n\nLast logs:\n" + getLastLogs();
    info += "\n\nStack memory:\n";
