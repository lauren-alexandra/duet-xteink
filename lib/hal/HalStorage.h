...
class HalStorage {
 public:
  ...
  // Read-side existence check with legacy Duet namespace fallback. Mutation
  // code must continue to use exists(), which only inspects the canonical path.
  bool existsForRead(const char* path);
  bool existsForRead(const std::string& path);
