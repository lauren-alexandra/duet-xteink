...
struct BmpConvertCtx {
  ...
  uint8_t* bmpRow;
  JpegToBmpConverter::CancelCallback shouldCancel;
  void* cancelContext;

  std::unique_ptr<AtkinsonDitherer> atkinsonDitherer;
  std::unique_ptr<FloydSteinbergDitherer> fsDitherer;
  std::unique_ptr<Burkes1BitDitherer> burkes1BitDitherer;
...
static void writeOutputRow(BmpConvertCtx* ctx, const uint8_t* srcRow, int outY) {
  ...
  if (USE_8BIT_OUTPUT && !ctx->oneBit) {
    ...
    const bool reverse = ctx->burkes1BitDitherer && ctx->burkes1BitDitherer->isReverseRow();
    const int start = reverse ? ctx->outWidth - 1 : 0;
    const int end = reverse ? -1 : ctx->outWidth;
    const int step = reverse ? -1 : 1;
    for (int x = start; x != end; x += step) {
      const uint8_t bit = ctx->burkes1BitDitherer ? ctx->burkes1BitDitherer->processPixel(srcRow[x], x)
                                                  : quantize1bit(srcRow[x], x, outY);
      ctx->bmpRow[x / 8] |= (bit << (7 - (x % 8)));
    }
    if (ctx->burkes1BitDitherer) ctx->burkes1BitDitherer->nextRow();
  } else {
    for (int x = 0; x < ctx->outWidth; x++) {
...
static void flushScaledRow(BmpConvertCtx* ctx) {
  ...
  if (USE_8BIT_OUTPUT && !ctx->oneBit) {
    ...
    for (int x = start; x != end; x += step) {
      const uint8_t gray = (ctx->rowCount[x] > 0) ? (ctx->rowAccum[x] / ctx->rowCount[x]) : 0;
      const uint8_t bit = ctx->burkes1BitDitherer ? ctx->burkes1BitDitherer->processPixel(gray, x)
...
int bmpDrawCallback(JPEGDRAW* pDraw) {
  ...
  if (ctx->shouldCancel && ctx->shouldCancel(ctx->cancelContext)) {
    ctx->error = true;
    return 0;
  }
...
bool JpegToBmpConverter::jpegFileToBmpStreamInternal(FsFile& jpegFile, Print& bmpOut, int targetWidth, int targetHeight,
  ...
  if (shouldCancel && shouldCancel(cancelContext)) {
    LOG_DBG("JPG", "JPEG conversion cancelled before decode");
    return false;
  }
  ...
  ctx.cancelContext = cancelContext;
  ...
  if (oneBit) {
    ctx.burkes1BitDitherer = makeUniqueNoThrow<Burkes1BitDitherer>(outWidth);
    if (!ctx.burkes1BitDitherer || !ctx.burkes1BitDitherer->valid()) {
      LOG_ERR("JPG", "OOM: Burkes1BitDitherer");
      return false;
    }
  ...
  if (shouldCancel && shouldCancel(cancelContext)) {
    LOG_DBG("JPG", "JPEG conversion cancelled before pixel decode");
    return false;
  }
