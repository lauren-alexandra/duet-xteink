...
void TextBlock::render(const GfxRenderer& renderer, const int fontId, const int x, const int y,
  ...
  for (size_t i = 0; i < words.size(); i++) {
    ...
    if (boundary > 0) {
      ...
      if (!scanning && !renderer.hasDistinctFontStyle(fontId, currentStyle, boldStyle)) {
        renderer.drawText(fontId, wordX + 1, wordY, boldBuf, foregroundBlack, boldStyle, baseDir);
      }
