...
struct SectionBuildOptions {
  ...
  bool* cancelledByCaller = nullptr;
  bool skipFontPrewarm = false;
  uint32_t fontPrewarmByteLimit = 0;

  bool isPreview() const {
    const bool hasAnchor = previewAnchor && previewAnchor[0] != '\0';
    return previewMaxPages > 0 && (hasAnchor || previewParagraphIndex != UINT16_MAX);
  }
  bool cancellationRequested() const { return shouldCancel && shouldCancel(cancelContext); }
};
...
class Section {
  ...
  bool writeSectionFileHeader(int fontId, float lineCompression, bool extraParagraphSpacing, bool forceParagraphIndents,
                              ...
                              uint8_t bionicReadingMode, bool guideReadingEnabled, EpubRenderMode renderMode);
 ...
 public:
  ...
  bool loadSectionFile(int fontId, float lineCompression, bool extraParagraphSpacing, bool forceParagraphIndents,
                       uint8_t paragraphAlignment, uint16_t viewportWidth, uint16_t viewportHeight,
                       bool hyphenationEnabled, bool embeddedStyle, uint8_t imageRendering, uint8_t bionicReadingMode,
