...
constexpr size_t DEFAULT_BUFFERED_WORDS_BEFORE_LAYOUT = 160;
constexpr uint16_t DEFAULT_TEXT_RUN_BYTES_BEFORE_LAYOUT = 1024;
constexpr size_t SINGLE_READING_AID_BUFFERED_WORDS_BEFORE_LAYOUT = 120;
constexpr uint16_t SINGLE_READING_AID_TEXT_RUN_BYTES_BEFORE_LAYOUT = 768;
constexpr size_t COMBINED_READING_AID_BUFFERED_WORDS_BEFORE_LAYOUT = 90;
constexpr uint16_t COMBINED_READING_AID_TEXT_RUN_BYTES_BEFORE_LAYOUT = 640;
...
constexpr uint32_t SECTION_ADVANCE_PREWARM_BATCH_CODEPOINTS = 32;
...
void ChapterHtmlSlimParser::applyFontVariantToEntry(StyleStackEntry& entry, const CssStyle& css) {
  if (css.hasFontVariantCaps()) {
    entry.hasFontVariantCaps = true;
    entry.fontVariantCaps = css.fontVariantCaps;
  }
}
...
void ChapterHtmlSlimParser::updateEffectiveInlineStyle() {
  ...
  for (size_t i = 0; i < inlineStyleCount_; ++i) {
    ...
    if (entry.hasFontVariantCaps) {
      effectiveFontVariantCaps = entry.fontVariantCaps;
    }
...
bool ChapterHtmlSlimParser::handlePreviewScanStart(const XML_Char* name, const XML_Char** atts) {
  if (!previewAnchor.empty()) {
    const char* idValue = getAttribute(atts, "id");
    if (idValue && strcmp(idValue, previewAnchor.c_str()) == 0) {
      startPreviewAtTarget();
      return false;
    }
  } else if (previewParagraphIndex != UINT16_MAX && strcmp(name, "p") == 0) {
    xpathParagraphIndex++;
    if (xpathParagraphIndex == previewParagraphIndex) {
      previewParagraphStartConsumed = true;
      startPreviewAtTarget();
      return false;
    }
  }
...
void ChapterHtmlSlimParser::startPreviewAtTarget() {
  previewTargetFound = true;
  ...
  if (!previewAnchor.empty()) {
    ...
    LOG_DBG("EHP", "Started preview at paragraph %u", previewParagraphIndex);
  }
}

void ChapterHtmlSlimParser::stopPreviewIfPageLimitReached() {
  if (!isPreviewBuild() || !previewTargetFound || previewStopRequested || completedPageCount < previewMaxPages) {
    return;
  }
  ...
  LOG_DBG("EHP", "Stopping preview after %u pages", previewMaxPages);
}
...
void ChapterHtmlSlimParser::flushPartWordBuffer() {
  ...
  if (effectiveFontVariantCaps == CssFontVariantCaps::SmallCaps) {
    fontStyle = static_cast<EpdFontFamily::Style>(fontStyle | EpdFontFamily::SMALL_CAPS);
  } else if (effectiveFontVariantCaps == CssFontVariantCaps::AllSmallCaps) {
    fontStyle = static_cast<EpdFontFamily::Style>(fontStyle | EpdFontFamily::ALL_SMALL_CAPS);
  }
...
void XMLCALL ChapterHtmlSlimParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (self->isScanningForPreviewTarget()) {
    if (self->handlePreviewScanStart(name, atts)) {
      return;
    }
  }
  ...
  if (strcmp(name, "p") == 0) {
    if (self->previewParagraphStartConsumed) {
      self->previewParagraphStartConsumed = false;
    } else {
  ...
  if (atts != nullptr) {
    for (int i = 0; atts[i]; i += 2) {
      ...
      if (strcmp(atts[i], "class") == 0) {
        ...
        if (self->isPreviewBuild() && self->previewTargetFound && strcmp(attrValue, self->previewAnchor.c_str()) == 0) {
          continue;
        }
  ...
  if (self->tableDepth == 1 && (strcmp(name, "td") == 0 || strcmp(name, "th") == 0)) {
    ...
    if (self->currentTableCellIsHeader) {
      ...
      ChapterHtmlSlimParser::applyFontVariantToEntry(headerStyle, cssStyle);
      if (self->inlineStyleCount_ < MAX_INLINE_STYLE_DEPTH) {
        self->inlineStyleBuf_[self->inlineStyleCount_++] = headerStyle;
      } else {
  ...
  if (strcmp(name, "a") == 0) {
    ...
    if (isInternalLink) {
      ...
      applyFontVariantToEntry(entry, cssStyle);
      if (self->inlineStyleCount_ < MAX_INLINE_STYLE_DEPTH) {
  ...
  if (matches(name, HEADER_TAGS, std::size(HEADER_TAGS))) {
    ...
    ChapterHtmlSlimParser::applyFontVariantToEntry(entry, cssStyle);
    if (self->inlineStyleCount_ < MAX_INLINE_STYLE_DEPTH) {
      self->inlineStyleBuf_[self->inlineStyleCount_++] = entry;
    } else {
    ...
    if (cssStyle.hasFontWeight() || cssStyle.hasFontStyle() || cssStyle.hasFontVariantCaps() ||
        cssStyle.hasTextDecoration() || cssStyle.hasBackgroundBlack() || cssStyle.hasVerticalAlign() ||
...
void XMLCALL ChapterHtmlSlimParser::characterData(void* userData, const XML_Char* s, const int len) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  if (self->isScanningForPreviewTarget() || self->previewStopRequested) {
    return;
  }
  if (self->tableDepth == 1) {
...
void ChapterHtmlSlimParser::prewarmSectionAdvanceTable(FsFile& file) {
  if (skipFontPrewarm_ || !renderer.isSdCardFont(fontId)) {
    return;
  }
  ...
  while (file.available() > 0 && !hitCap && (fontPrewarmByteLimit_ == 0 || bytesScanned < fontPrewarmByteLimit_)) {
    ...
    bytesScanned += static_cast<uint32_t>(len);
  ...
  for (uint32_t offset = 0; offset < cpCount; offset += SECTION_ADVANCE_PREWARM_BATCH_CODEPOINTS) {
    if (cancellationRequested()) return;
    const uint32_t batchCount = std::min<uint32_t>(SECTION_ADVANCE_PREWARM_BATCH_CODEPOINTS, cpCount - offset);
    renderer.ensureSdCardFontReady(fontId, codepoints.get() + offset, batchCount, /*includeSpace=*/offset == 0,
                                   hyphenationEnabled && offset == 0, /*styleMask=*/0x0F);
  }
  LOG_DBG("EHP", "Section advance prewarm: bytes=%lu codepoints=%u time=%lu ms free=%u maxAlloc=%u",
          static_cast<unsigned long>(bytesScanned), static_cast<unsigned>(cpCount), millis() - startMs,
          ESP.getFreeHeap(), ESP.getMaxAllocHeap());
}

bool ChapterHtmlSlimParser::cancellationRequested() {
  if (cancelledByCaller_) return true;
  if (!shouldCancel_ || !shouldCancel_(cancelContext_)) return false;
  cancelledByCaller_ = true;
  return true;
}
...
bool ChapterHtmlSlimParser::beginParse() {
  ...
  if (isPreviewBuild() && previewParagraphIndex == 0) {
    // A paragraph index of zero means "preview from the true start of the
    // document". This keeps headings, chapter art, and other content before
    // the first <p> in a new-chapter preview.
    startPreviewAtTarget();
...
ChapterHtmlSlimParser::ParseStatus ChapterHtmlSlimParser::parseStep() {
  if (cancellationRequested()) return ParseStatus::Error;
  if (!activeParser) {
...
bool ChapterHtmlSlimParser::finishParse() {
  ...
  if (isPreviewBuild() && !previewTargetFound) {
    if (!previewAnchor.empty()) {
      LOG_ERR("EHP", "Preview anchor '%s' was not found", previewAnchor.c_str());
    } else {
      LOG_ERR("EHP", "Preview paragraph %u was not found", previewParagraphIndex);
    }
    abortParse();
    return false;
  }
