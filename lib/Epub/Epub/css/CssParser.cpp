...
bool CssParser::peekCacheStatus(bool& partial, bool& partialDueToRuleCap) {
  partial = false;
  partialDueToRuleCap = false;
  if (cachePath.empty()) {
    return false;
  }
  ...
  uint8_t version = 0;
  uint8_t flags = 0;
  const bool headerOk = file.read(&magic, sizeof(magic)) == sizeof(magic) && magic == CssParser::CSS_CACHE_MAGIC &&
                        file.read(&version, 1) == 1 && version == CssParser::CSS_CACHE_VERSION &&
                        file.read(&flags, 1) == 1 &&
                        (flags & ~(CSS_CACHE_FLAG_PARTIAL | CSS_CACHE_FLAG_PARTIAL_RULE_CAP)) == 0;
  file.close();
  if (!headerOk) {
    LOG_DBG("CSS", "Cache header invalid on peek, removing stale cache for rebuild");
    Storage.remove((cachePath + rulesCache).c_str());
    return false;
  }

  partial = (flags & CSS_CACHE_FLAG_PARTIAL) != 0;
  partialDueToRuleCap = (flags & CSS_CACHE_FLAG_PARTIAL_RULE_CAP) != 0;
  return true;
}
...
bool CssParser::saveToCache(const bool complete) const {
  ...
  writeByte(static_cast<uint8_t>(
      complete ? 0 : (CSS_CACHE_FLAG_PARTIAL | (ruleCapReached_ ? CSS_CACHE_FLAG_PARTIAL_RULE_CAP : 0))));
...
bool CssParser::loadFromCache() {
  ...
  if ((flags & ~(CSS_CACHE_FLAG_PARTIAL | CSS_CACHE_FLAG_PARTIAL_RULE_CAP)) != 0) {
  ...
  cachePartialRuleCap_ = (flags & CSS_CACHE_FLAG_PARTIAL_RULE_CAP) != 0;
