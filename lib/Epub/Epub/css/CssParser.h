...
class CssParser {
 public:
  ...
  void clear() {
    ...
    cachedRuleCount_ = 0;
    cachePartial_ = false;
    cachePartialRuleCap_ = false;
    ruleCapReached_ = false;
  }
  ...
  /**
   * True when the cache is partial because the MAX_RULES cap was hit — a
   * deterministic limit, so re-parsing the same CSS can never complete.
   */
  [[nodiscard]] bool isCachePartialDueToRuleCap() const { return cachePartialRuleCap_; }

  /**
   * Read only the cache header (magic/version/flags) without hydrating rules.
   * Removes the cache file when the header is invalid, mirroring
   * loadFromCache()'s stale-cache handling. Returns false when absent/invalid.
   */
  bool peekCacheStatus(bool& partial, bool& partialDueToRuleCap);

 private:
  static constexpr uint8_t CSS_CACHE_FLAG_PARTIAL = 1 << 0;
  // Set alongside CSS_CACHE_FLAG_PARTIAL when the parse stopped at MAX_RULES.
  // Older firmware rejects unknown flag bits and rebuilds the cache, so this
  // bit is backward-safe without a cache version bump.
  static constexpr uint8_t CSS_CACHE_FLAG_PARTIAL_RULE_CAP = 1 << 1;
  ...
  bool cachePartialRuleCap_ = false;
  // Set during parsing when MAX_RULES stops the parse; persisted via
  // CSS_CACHE_FLAG_PARTIAL_RULE_CAP so later opens skip futile rebuilds.
  bool ruleCapReached_ = false;
