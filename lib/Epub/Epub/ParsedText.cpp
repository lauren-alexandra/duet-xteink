...
BionicTokenMetadata computeBionicMetadata(const std::string_view segment, const EpdFontFamily::Style baseStyle,
  ...
  if (BionicReadingMode::normalize(bionicReadingMode) == BionicReadingMode::Off ||
      (baseStyle & EpdFontFamily::BOLD) != 0 || segment.empty()) {
    return {baseStyle, 0};
  }
  ...
  // Normal keeps the existing 43% fixation. Subtle reduces emphasis on longer words.
  size_t targetBoldChars = (charCount * BionicReadingMode::boldPrefixPercent(bionicReadingMode)) / 100;
...
int measureBionicSuffixX(const GfxRenderer& renderer, const int fontId, const std::string& word,
  ...
  const int syntheticBoldWidth = renderer.hasDistinctFontStyle(fontId, style, boldStyle) ? 0 : 1;
  return prefixWidth + kern + syntheticBoldWidth;
}
...
void ParsedText::addWord(std::string word, const EpdFontFamily::Style fontStyle, const bool underline,
  ...
  if (BionicReadingMode::normalize(this->bionicReadingEnabled) == BionicReadingMode::Off ||
      (baseStyle & EpdFontFamily::BOLD) != 0) {
    pushToken(std::move(word), effectiveAttachToPrevious, effectiveNoSpaceBefore, baseStyle, 0);
    if (wordStartsRtl) {
      hasRtlWord = true;
    }
...
bool ParsedText::layoutAndExtractLines(const GfxRenderer& renderer, const int fontId, const uint16_t viewportWidth,
                                       ...
                                       const bool includeLastLine, const CancelCallback shouldCancel,
                                       void* cancelContext) {
  const auto cancellationRequested = [&]() { return shouldCancel && shouldCancel(cancelContext); };
  if (words.empty()) {
    return true;
  }
  ...
  if (renderer.isSdCardFont(fontId)) {
    ...
    for (size_t i = 0; i < wordStyles.size(); ++i) {
      if ((i & 0x1F) == 0 && cancellationRequested()) return false;
  ...
  if (!calculateWordWidths(wordWidths, renderer, fontId, shouldCancel, cancelContext)) {
  ...
  if (hyphenationEnabled) {
    ...
    breaksOk = computeHyphenatedLineBreaks(renderer, fontId, pageWidth, wordWidths, wordContinues, wordNoSpaceBefore,
                                           lineBreakIndices, shouldCancel, cancelContext);
    if (breaksOk) {
    ...
    breaksOk = computeLineBreaks(layoutArena, renderer, fontId, pageWidth, wordWidths, wordContinues, wordNoSpaceBefore,
                                 naturalGaps, gapSlots, lineBreakIndices, shouldCancel, cancelContext);
  }
  ...
  for (size_t i = 0; i < lineCount; ++i) {
    if ((i & 0x07) == 0 && cancellationRequested()) return false;
...
bool ParsedText::layoutAndExtractLinesPreservingSource(
    ...
    const std::function<void(std::shared_ptr<TextBlock>)>& processLine, const CancelCallback shouldCancel,
    void* cancelContext) const {
  ParsedText layoutProbe(*this);
  return layoutProbe.layoutAndExtractLines(renderer, fontId, viewportWidth, processLine, true, shouldCancel,
                                           cancelContext);
}

bool ParsedText::calculateWordWidths(ArenaVector<uint16_t>& wordWidths, const GfxRenderer& renderer, const int fontId,
                                     const CancelCallback shouldCancel, void* cancelContext) {
  if (!wordWidths.reserve(words.size())) {
    return false;
  }

  for (size_t i = 0; i < words.size(); ++i) {
    if ((i & 0x1F) == 0 && shouldCancel && shouldCancel(cancelContext)) return false;
...
bool ParsedText::computeLineBreaks(Arena& scratchArena, const GfxRenderer& renderer, const int fontId,
                                   ...
                                   ArenaVector<size_t>& lineBreakIndices, const CancelCallback shouldCancel,
                                   void* cancelContext) {
  if (words.empty()) {
    return false;
  }
  ...
  for (size_t i = 0; i < wordWidths.size(); ++i) {
    if ((i & 0x0F) == 0 && shouldCancel && shouldCancel(cancelContext)) return false;
  ...
  for (int i = totalWordCount - 2; i >= 0; --i) {
    if ((static_cast<size_t>(i) & 0x0F) == 0 && shouldCancel && shouldCancel(cancelContext)) return false;
    int currlen = 0;
    dp[i] = MAX_COST;
    ...
    for (size_t j = i; j < totalWordCount; ++j) {
      if (((j - static_cast<size_t>(i)) & 0x1F) == 0 && shouldCancel && shouldCancel(cancelContext)) return false;
  ...
  while (currentWordIndex < totalWordCount) {
    if (shouldCancel && shouldCancel(cancelContext)) return false;
...
bool ParsedText::computeHyphenatedLineBreaks(const GfxRenderer& renderer, const int fontId, const int pageWidth,
  ...
  while (currentIndex < wordWidths.size()) {
    ...
    while (currentIndex < wordWidths.size()) {
      if (((currentIndex - lineStart) & 0x1F) == 0 && shouldCancel && shouldCancel(cancelContext)) return false;
