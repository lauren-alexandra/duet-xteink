...
uint16_t initialPageLutCapacity(const SectionBuildOptions& buildOptions) {
  if (!buildOptions.isPreview()) {
    return INITIAL_SECTION_PAGE_LUT_ENTRIES;
  }
  return std::max<uint16_t>(MIN_PREVIEW_PAGE_LUT_ENTRIES,
                            std::min<uint16_t>(INITIAL_SECTION_PAGE_LUT_ENTRIES, buildOptions.previewMaxPages));
}
...
Section::Section(const std::shared_ptr<Epub>& epub, const int spineIndex, GfxRenderer& renderer,
    ...
    : epub(epub), spineIndex(spineIndex), renderer(renderer), filePath() {
  epub->setupLayoutCacheDir();
  filePath = epub->getLayoutCachePath() + "/sections/" + std::to_string(spineIndex) + (cacheSuffix ? cacheSuffix : "") +
             ".bin";
}
...
bool Section::writeSectionFileHeader(const int fontId, const float lineCompression, const bool extraParagraphSpacing,
                                     ...
                                     const uint8_t imageRendering, const uint8_t bionicReadingMode,
  ...
  static_assert(HEADER_SIZE == sizeof(SECTION_CACHE_MAGIC) + sizeof(SECTION_FILE_VERSION) + sizeof(fontId) +
                                   ...
                                   sizeof(embeddedStyle) + sizeof(imageRendering) + sizeof(bionicReadingMode) +
  ...
  return serialization::tryWritePod(file, SECTION_CACHE_MAGIC) &&
         ...
         serialization::tryWritePod(file, bionicReadingMode) && serialization::tryWritePod(file, guideReadingEnabled) &&
...
bool Section::loadSectionFile(const int fontId, const float lineCompression, const bool extraParagraphSpacing,
                              ...
                              const uint8_t bionicReadingMode, const bool guideReadingEnabled,
  ...
  // Match parameters
  {
    ...
    if (fontId != fileFontId || lineCompression != fileLineCompression ||
        ...
        bionicReadingMode != fileBionicReadingMode || guideReadingEnabled != fileGuideReadingEnabled ||
...
bool Section::createSectionFile(const int fontId, const float lineCompression, const bool extraParagraphSpacing,
  ...
  const uint32_t createStartMs = millis();
  if (buildOptions.cancelledByCaller) *buildOptions.cancelledByCaller = false;
  const auto cancellationRequested = [&]() {
    if (!buildOptions.cancellationRequested()) return false;
    if (buildOptions.cancelledByCaller) *buildOptions.cancelledByCaller = true;
    return true;
  };
  if (cancellationRequested()) return false;

  const auto localPath = epub->getSpineItem(spineIndex).href;
  const auto htmlDir = epub->getLayoutCachePath() + "/html";
  ...
  const uint8_t effectiveBionicReadingMode = bionicReadingMode;
  ...
  LOG_DBG("SCT",
          ...
          viewportHeight, imageRendering, effectiveBionicReadingMode, effectiveGuideReadingEnabled, ESP.getFreeHeap(),
          ESP.getMaxAllocHeap());

  // Create cache directory if it doesn't exist
  {
    const auto sectionsDir = epub->getLayoutCachePath() + "/sections";
  ...
  if (reusedHtml) {
    ...
    for (int attempt = 0; attempt < 3 && !streamed; attempt++) {
      ...
      const size_t htmlStreamChunkSize = sectionHtmlStreamChunkSize(buildOptions.isPreview());
      {
        ...
        streamed = epub->readItemContentsToStream(localPath, tmpHtml, htmlStreamChunkSize, buildOptions.shouldCancel,
                                                  buildOptions.cancelContext);
      }
  ...
  const uint32_t htmlReadyMs = millis();
  if (cancellationRequested()) {
    cleanupTempHtml();
    return false;
  }
  ...
  if (!writeSectionFileHeader(fontId, lineCompression, extraParagraphSpacing, forceParagraphIndents, paragraphAlignment,
                              ...
                              effectiveBionicReadingMode, effectiveGuideReadingEnabled, renderMode)) {
  ...
  // Most chapters are far below 1024 pages. Start small and grow geometrically
  // so ordinary chapters and short previews do not reserve 8 KB up front.
  uint16_t lutCapacity = initialPageLutCapacity(buildOptions);
  ...
  std::string imageBasePath = epub->getLayoutCachePath() + "/img_" + std::to_string(spineIndex) + "_";
  ...
  if (embeddedStyle) {
    ...
    if (cssParser) {
      ...
      if (!cssLoaded && cssParser->hasCache()) {
        // The open path validates only the cache header now, so a payload
        // corruption surfaces here; remove the file so the next open rebuilds.
        LOG_ERR("SCT", "Failed to load CSS from cache; deleting it for rebuild");
  ...
  ChapterHtmlSlimParser visitor(
      ...
      paragraphAlignment, viewportWidth, viewportHeight, hyphenationEnabled, effectiveBionicReadingMode,
      ...
      buildOptions.isPreview() && buildOptions.previewAnchor ? std::string(buildOptions.previewAnchor) : std::string{},
      buildOptions.previewParagraphIndex, buildOptions.previewMaxPages, buildOptions.shouldCancel,
      buildOptions.cancelContext, buildOptions.skipFontPrewarm, buildOptions.fontPrewarmByteLimit);
  Hyphenator::setPreferredLanguage(epub->getLanguage());
  LOG_DBG("SCT", "Parser start: spine=%d free=%u maxAlloc=%u", spineIndex, ESP.getFreeHeap(), ESP.getMaxAllocHeap());
  const uint32_t parserStartMs = millis();
  const bool success = visitor.parseAndBuildPages();
  const uint32_t parserDoneMs = millis();
  if (visitor.wasCancelledByCaller() && buildOptions.cancelledByCaller) {
    *buildOptions.cancelledByCaller = true;
  }
  ...
  if (!success || pageCompletionFailed) {
    if (!visitor.wasCancelledByCaller()) {
...
bool Section::hasHtmlCache() const {
  const std::string htmlPath = epub->getLayoutCachePath() + "/html/" + std::to_string(spineIndex) + ".html";
